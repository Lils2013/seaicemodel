...
*	                        ! other proposals see in Mellor and Yamada, 1982
      Parameter (cLmin=1.0  ) ! Minimum length scale
...
c      if(1.-aice(0,i,j) .GT. Aimin) then
c      himean= himean/(1.-aice(0,i,j))
c	else
c	himean=0.
c	end if
...
*     Mean Drag coefficient
	...
	windx= windx +drag*(1.-Aopen)*(uice(i,j)-u(i,j,1))
	windy= windy +drag*(1.-Aopen)*(vice(i,j)-v(i,j,1))

	u_star= SQRT(sqrt(windx**2 + windy**2)/row)  !!! Friction velocity

	cL0= 0.03*u_star/abs(2.*om*co(i)*S0)

      do k=1,kb

*     Calculate master length scale

      cLs= max(1.e-1, z(k))          ! distance to the surface
	cLb= max(1.e-1, z(kb)-z(k))    ! distance to the bottom
	cLd= cLs*cLb/(cLs+cLb)
...
*     coefficients and right hand side for vertical sweeping.
	...
	if(k.eq.2) FM(k)= FM(k) -AM(2)*q2turb(i,j,1)
