...
*******************************************************
      ...
      parameter (gamma_fct=2.e-1)         ! Numerical scalar diffusivity
...
*     Auxiliary arrays for FCT calculations
	...
	common /consts/ c3,c6,c12,c18,Rhx,R2hx2
      INCLUDE 'Tparm.fi'
...
*     Antidiffusive flux input in the node 
      ...
      if(Pplus. GT. 0.) then
      Rplus(i,j,k) = min(1., Qplus/max(1.e-19,Pplus))
	else
	Rplus(i,j,k) = 0.
	end if

      if(Pminus. LT. 0.) then
      Rminus(i,j,k) = min(1., Qminus/min(-1.e-19,Pminus))
	else
	Rminus(i,j,k) = 0.
	end if
