...
c     Version 19.01.2013 for ice thickness gradations.
...
*     side melting - see IceTH).
	...
	if(dHiceT .GT. 1.e6) write(*,*) i,j,dHiceT
...
*     ------------- Matrix factorization ---------
	...
	if(RKSI(k) .LE. -1.) then
	write(*,*) 
     *i,j,k,RKSI(k),am(k),bm(k),cm(k),fm(k),S(i,j,k),dHiceT
	end if
...
c     Total heat flux:
	...
	write(*,*) '============================='
	write(*,*)'Snow temp.!', T1,T2,F2,i,j,m,TA(i,j),hs
	write(*,*) '============================='
	end if
...
c     &sigma*(TAK**4)*(0.67+0.005*sqrt(epsa))*(1.+0.29*cloud(i,j))
	...
	if(iter.eq.miter.and.abs(f2).GT.accur)then
	write(*,*) '============================='
	write(*,*)'Ice temp.!', T1,T2,F2,i,j,m,TA(i,j)
	write(*,*) '============================='
	end if
...
cc      ai= 0.55  ! Boulder Ice CCSM3 (Ice version 5, 2004) 
