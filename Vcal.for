...
*     Gravity Wave Drag for Ice-Ocean
*
*     Version 14.12.2014.
*
******************************************************
...
*     ---------------------------------------
	...
	delta_u= SQRT((uice(i,j)-u(i,j,1))**2+(vice(i,j)-v(i,j,1))**2)

      delta_u= MAX(1.e-3, delta_u) ! Regularization !!!!
      delta_u= MIN(1.e+2, delta_u) ! Regularization !!!!
      

*     Mean Drag coefficient under ice
	drag=( row*Cdw*delta_u +CDgwd(i,j) )*(1.-Aopen)
...
CCC	double precision dzmean, dzmmean
	...
	c24= 0.5*c12
      ...
      asr=hx/hy
	asr2=asr*asr
      A=.5*AL*HXX*HXX/R2  ! For Standart Galerkin Scheme.
...
*     -------------- Surface -----------------
      ...
      Sum1= c24*
     * (KT(1,n)*(2.*um1(i,j,1)+um1(i+1,j,1)+um1(i+1,j-1,1))
     + +KT(2,n)*(2.*um1(i,j,1)+um1(i,j-1,1)+um1(i+1,j-1,1))
     + +KT(3,n)*(2.*um1(i,j,1)+um1(i,j-1,1)+um1(i-1,j  ,1))
     + +KT(4,n)*(2.*um1(i,j,1)+um1(i-1,j,1)+um1(i-1,j+1,1))
     + +KT(5,n)*(2.*um1(i,j,1)+um1(i-1,j+1,1)+um1(i,j+1,1))
     + +KT(6,n)*(2.*um1(i,j,1)+um1(i,j+1,1)+um1(i+1,j  ,1)))

      Sum2= c24*
     * (KT(1,n)*(2.*um1(i,j,2)+um1(i+1,j,2)+um1(i+1,j-1,2))
     + +KT(2,n)*(2.*um1(i,j,2)+um1(i,j-1,2)+um1(i+1,j-1,2))
     + +KT(3,n)*(2.*um1(i,j,2)+um1(i,j-1,2)+um1(i-1,j  ,2))
     + +KT(4,n)*(2.*um1(i,j,2)+um1(i-1,j,2)+um1(i-1,j+1,2))
     + +KT(5,n)*(2.*um1(i,j,2)+um1(i-1,j+1,2)+um1(i,j+1,2))
     + +KT(6,n)*(2.*um1(i,j,2)+um1(i,j+1,2)+um1(i+1,j  ,2)))

      FZu=-0.5*S0*((Sum1+Sum2)*W(i,j,2) -2.*Sum1*(W(i,j,1)-PME(i,j)/dt))

      Sum1= c24*
     * (KT(1,n)*(2.*vm1(i,j,1)+vm1(i+1,j,1)+vm1(i+1,j-1,1))
     + +KT(2,n)*(2.*vm1(i,j,1)+vm1(i,j-1,1)+vm1(i+1,j-1,1))
     + +KT(3,n)*(2.*vm1(i,j,1)+vm1(i,j-1,1)+vm1(i-1,j  ,1))
     + +KT(4,n)*(2.*vm1(i,j,1)+vm1(i-1,j,1)+vm1(i-1,j+1,1))
     + +KT(5,n)*(2.*vm1(i,j,1)+vm1(i-1,j+1,1)+vm1(i,j+1,1))
     + +KT(6,n)*(2.*vm1(i,j,1)+vm1(i,j+1,1)+vm1(i+1,j  ,1)))

      Sum2= c24*
     * (KT(1,n)*(2.*vm1(i,j,2)+vm1(i+1,j,2)+vm1(i+1,j-1,2))
     + +KT(2,n)*(2.*vm1(i,j,2)+vm1(i,j-1,2)+vm1(i+1,j-1,2))
     + +KT(3,n)*(2.*vm1(i,j,2)+vm1(i,j-1,2)+vm1(i-1,j  ,2))
     + +KT(4,n)*(2.*vm1(i,j,2)+vm1(i-1,j,2)+vm1(i-1,j+1,2))
     + +KT(5,n)*(2.*vm1(i,j,2)+vm1(i-1,j+1,2)+vm1(i,j+1,2))
     + +KT(6,n)*(2.*vm1(i,j,2)+vm1(i,j+1,2)+vm1(i+1,j  ,2)))

      FZv=-0.5*S0*((Sum1+Sum2)*W(i,j,2)-2.*Sum1*(W(i,j,1)-PME(i,j)/dt))
...
*     Metric in Transport
	...
	Transp_Metric_V= +Um2(i,j,1)*Um1(i,j,1)*COS_A/(R*S0)
      ...
      Sum1= c24*
     * (KT(1,n)*(2.*um2(i,j,1)+um2(i+1,j,1)+um2(i+1,j-1,1))
     + +KT(2,n)*(2.*um2(i,j,1)+um2(i,j-1,1)+um2(i+1,j-1,1))
     + +KT(3,n)*(2.*um2(i,j,1)+um2(i,j-1,1)+um2(i-1,j  ,1))
     + +KT(4,n)*(2.*um2(i,j,1)+um2(i-1,j,1)+um2(i-1,j+1,1))
     + +KT(5,n)*(2.*um2(i,j,1)+um2(i-1,j+1,1)+um2(i,j+1,1))
     + +KT(6,n)*(2.*um2(i,j,1)+um2(i,j+1,1)+um2(i+1,j  ,1)))

      Sum2= c24*
     * (KT(1,n)*(2.*um2(i,j,2)+um2(i+1,j,2)+um2(i+1,j-1,2))
     + +KT(2,n)*(2.*um2(i,j,2)+um2(i,j-1,2)+um2(i+1,j-1,2))
     + +KT(3,n)*(2.*um2(i,j,2)+um2(i,j-1,2)+um2(i-1,j  ,2))
     + +KT(4,n)*(2.*um2(i,j,2)+um2(i-1,j,2)+um2(i-1,j+1,2))
     + +KT(5,n)*(2.*um2(i,j,2)+um2(i-1,j+1,2)+um2(i,j+1,2))
     + +KT(6,n)*(2.*um2(i,j,2)+um2(i,j+1,2)+um2(i+1,j  ,2)))

      FZu=-0.5*S0*((Sum1+Sum2)*W(i,j,2)-2.*Sum1*(W(i,j,1)-PME(i,j)/dt))

      Sum1= c24*
     * (KT(1,n)*(2.*vm2(i,j,1)+vm2(i+1,j,1)+vm2(i+1,j-1,1))
     + +KT(2,n)*(2.*vm2(i,j,1)+vm2(i,j-1,1)+vm2(i+1,j-1,1))
     + +KT(3,n)*(2.*vm2(i,j,1)+vm2(i,j-1,1)+vm2(i-1,j  ,1))
     + +KT(4,n)*(2.*vm2(i,j,1)+vm2(i-1,j,1)+vm2(i-1,j+1,1))
     + +KT(5,n)*(2.*vm2(i,j,1)+vm2(i-1,j+1,1)+vm2(i,j+1,1))
     + +KT(6,n)*(2.*vm2(i,j,1)+vm2(i,j+1,1)+vm2(i+1,j  ,1)))

      Sum2= c24*
     * (KT(1,n)*(2.*vm2(i,j,2)+vm2(i+1,j,2)+vm2(i+1,j-1,2))
     + +KT(2,n)*(2.*vm2(i,j,2)+vm2(i,j-1,2)+vm2(i+1,j-1,2))
     + +KT(3,n)*(2.*vm2(i,j,2)+vm2(i,j-1,2)+vm2(i-1,j  ,2))
     + +KT(4,n)*(2.*vm2(i,j,2)+vm2(i-1,j,2)+vm2(i-1,j+1,2))
     + +KT(5,n)*(2.*vm2(i,j,2)+vm2(i-1,j+1,2)+vm2(i,j+1,2))
     + +KT(6,n)*(2.*vm2(i,j,2)+vm2(i,j+1,2)+vm2(i+1,j  ,2)))
...
*     ------------------ Interior ----------------
      ...
      Sum1= c24*
     * (KT(1,n)*(2.*um1(i,j,k)+um1(i+1,j,k)+um1(i+1,j-1,k))
     + +KT(2,n)*(2.*um1(i,j,k)+um1(i,j-1,k)+um1(i+1,j-1,k))
     + +KT(3,n)*(2.*um1(i,j,k)+um1(i,j-1,k)+um1(i-1,j  ,k))
     + +KT(4,n)*(2.*um1(i,j,k)+um1(i-1,j,k)+um1(i-1,j+1,k))
     + +KT(5,n)*(2.*um1(i,j,k)+um1(i-1,j+1,k)+um1(i,j+1,k))
     + +KT(6,n)*(2.*um1(i,j,k)+um1(i,j+1,k)+um1(i+1,j  ,k)))

      Sum1m= c24*
     * (KT(1,n)*(2.*um1(i,j,km)+um1(i+1,j,km)+um1(i+1,j-1,km))
     + +KT(2,n)*(2.*um1(i,j,km)+um1(i,j-1,km)+um1(i+1,j-1,km))
     + +KT(3,n)*(2.*um1(i,j,km)+um1(i,j-1,km)+um1(i-1,j  ,km))
     + +KT(4,n)*(2.*um1(i,j,km)+um1(i-1,j,km)+um1(i-1,j+1,km))
     + +KT(5,n)*(2.*um1(i,j,km)+um1(i-1,j+1,km)+um1(i,j+1,km))
     + +KT(6,n)*(2.*um1(i,j,km)+um1(i,j+1,km)+um1(i+1,j  ,km)))

      Sum2= c24*
     * (KT(1,np)*(2.*um1(i,j,k)+um1(i+1,j,k)+um1(i+1,j-1,k))
     + +KT(2,np)*(2.*um1(i,j,k)+um1(i,j-1,k)+um1(i+1,j-1,k))
     + +KT(3,np)*(2.*um1(i,j,k)+um1(i,j-1,k)+um1(i-1,j  ,k))
     + +KT(4,np)*(2.*um1(i,j,k)+um1(i-1,j,k)+um1(i-1,j+1,k))
     + +KT(5,np)*(2.*um1(i,j,k)+um1(i-1,j+1,k)+um1(i,j+1,k))
     + +KT(6,np)*(2.*um1(i,j,k)+um1(i,j+1,k)+um1(i+1,j  ,k)))

      Sum2p= c24*
     * (KT(1,np)*(2.*um1(i,j,kp)+um1(i+1,j,kp)+um1(i+1,j-1,kp))
     + +KT(2,np)*(2.*um1(i,j,kp)+um1(i,j-1,kp)+um1(i+1,j-1,kp))
     + +KT(3,np)*(2.*um1(i,j,kp)+um1(i,j-1,kp)+um1(i-1,j  ,kp))
     + +KT(4,np)*(2.*um1(i,j,kp)+um1(i-1,j,kp)+um1(i-1,j+1,kp))
     + +KT(5,np)*(2.*um1(i,j,kp)+um1(i-1,j+1,kp)+um1(i,j+1,kp))
     + +KT(6,np)*(2.*um1(i,j,kp)+um1(i,j+1,kp)+um1(i+1,j  ,kp)))

      FZU=-0.5*S0*( (Sum2+Sum2p)*W(i,j,Kp) -(Sum1+Sum1m)*W(i,j,K) )


      Sum1= c24*
     * (KT(1,n)*(2.*vm1(i,j,k)+vm1(i+1,j,k)+vm1(i+1,j-1,k))
     + +KT(2,n)*(2.*vm1(i,j,k)+vm1(i,j-1,k)+vm1(i+1,j-1,k))
     + +KT(3,n)*(2.*vm1(i,j,k)+vm1(i,j-1,k)+vm1(i-1,j  ,k))
     + +KT(4,n)*(2.*vm1(i,j,k)+vm1(i-1,j,k)+vm1(i-1,j+1,k))
     + +KT(5,n)*(2.*vm1(i,j,k)+vm1(i-1,j+1,k)+vm1(i,j+1,k))
     + +KT(6,n)*(2.*vm1(i,j,k)+vm1(i,j+1,k)+vm1(i+1,j  ,k)))

      Sum1m= c24*
     * (KT(1,n)*(2.*vm1(i,j,km)+vm1(i+1,j,km)+vm1(i+1,j-1,km))
     + +KT(2,n)*(2.*vm1(i,j,km)+vm1(i,j-1,km)+vm1(i+1,j-1,km))
     + +KT(3,n)*(2.*vm1(i,j,km)+vm1(i,j-1,km)+vm1(i-1,j  ,km))
     + +KT(4,n)*(2.*vm1(i,j,km)+vm1(i-1,j,km)+vm1(i-1,j+1,km))
     + +KT(5,n)*(2.*vm1(i,j,km)+vm1(i-1,j+1,km)+vm1(i,j+1,km))
     + +KT(6,n)*(2.*vm1(i,j,km)+vm1(i,j+1,km)+vm1(i+1,j  ,km)))

      Sum2= c24*
     * (KT(1,np)*(2.*vm1(i,j,k)+vm1(i+1,j,k)+vm1(i+1,j-1,k))
     + +KT(2,np)*(2.*vm1(i,j,k)+vm1(i,j-1,k)+vm1(i+1,j-1,k))
     + +KT(3,np)*(2.*vm1(i,j,k)+vm1(i,j-1,k)+vm1(i-1,j  ,k))
     + +KT(4,np)*(2.*vm1(i,j,k)+vm1(i-1,j,k)+vm1(i-1,j+1,k))
     + +KT(5,np)*(2.*vm1(i,j,k)+vm1(i-1,j+1,k)+vm1(i,j+1,k))
     + +KT(6,np)*(2.*vm1(i,j,k)+vm1(i,j+1,k)+vm1(i+1,j  ,k)))

      Sum2p= c24*
     * (KT(1,np)*(2.*vm1(i,j,kp)+vm1(i+1,j,kp)+vm1(i+1,j-1,kp))
     + +KT(2,np)*(2.*vm1(i,j,kp)+vm1(i,j-1,kp)+vm1(i+1,j-1,kp))
     + +KT(3,np)*(2.*vm1(i,j,kp)+vm1(i,j-1,kp)+vm1(i-1,j  ,kp))
     + +KT(4,np)*(2.*vm1(i,j,kp)+vm1(i-1,j,kp)+vm1(i-1,j+1,kp))
     + +KT(5,np)*(2.*vm1(i,j,kp)+vm1(i-1,j+1,kp)+vm1(i,j+1,kp))
     + +KT(6,np)*(2.*vm1(i,j,kp)+vm1(i,j+1,kp)+vm1(i+1,j  ,kp)))

      FZV=-0.5*S0*( (Sum2+Sum2p)*W(i,j,Kp) -(Sum1+Sum1m)*W(i,j,K) )
...
*     Metric in Transport
	...
	Transp_Metric_V= +Um2(i,j,k)*Um1(i,j,k)*COS_A/(R*S0)
      ...
      Sum1= c24*
     * (KT(1,n)*(2.*um2(i,j,k)+um2(i+1,j,k)+um2(i+1,j-1,k))
     + +KT(2,n)*(2.*um2(i,j,k)+um2(i,j-1,k)+um2(i+1,j-1,k))
     + +KT(3,n)*(2.*um2(i,j,k)+um2(i,j-1,k)+um2(i-1,j  ,k))
     + +KT(4,n)*(2.*um2(i,j,k)+um2(i-1,j,k)+um2(i-1,j+1,k))
     + +KT(5,n)*(2.*um2(i,j,k)+um2(i-1,j+1,k)+um2(i,j+1,k))
     + +KT(6,n)*(2.*um2(i,j,k)+um2(i,j+1,k)+um2(i+1,j  ,k)))

      Sum1m= c24*
     * (KT(1,n)*(2.*um2(i,j,km)+um2(i+1,j,km)+um2(i+1,j-1,km))
     + +KT(2,n)*(2.*um2(i,j,km)+um2(i,j-1,km)+um2(i+1,j-1,km))
     + +KT(3,n)*(2.*um2(i,j,km)+um2(i,j-1,km)+um2(i-1,j  ,km))
     + +KT(4,n)*(2.*um2(i,j,km)+um2(i-1,j,km)+um2(i-1,j+1,km))
     + +KT(5,n)*(2.*um2(i,j,km)+um2(i-1,j+1,km)+um2(i,j+1,km))
     + +KT(6,n)*(2.*um2(i,j,km)+um2(i,j+1,km)+um2(i+1,j  ,km)))

      Sum2= c24*
     * (KT(1,np)*(2.*um2(i,j,k)+um2(i+1,j,k)+um2(i+1,j-1,k))
     + +KT(2,np)*(2.*um2(i,j,k)+um2(i,j-1,k)+um2(i+1,j-1,k))
     + +KT(3,np)*(2.*um2(i,j,k)+um2(i,j-1,k)+um2(i-1,j  ,k))
     + +KT(4,np)*(2.*um2(i,j,k)+um2(i-1,j,k)+um2(i-1,j+1,k))
     + +KT(5,np)*(2.*um2(i,j,k)+um2(i-1,j+1,k)+um2(i,j+1,k))
     + +KT(6,np)*(2.*um2(i,j,k)+um2(i,j+1,k)+um2(i+1,j  ,k)))

      Sum2p= c24*
     * (KT(1,np)*(2.*um2(i,j,kp)+um2(i+1,j,kp)+um2(i+1,j-1,kp))
     + +KT(2,np)*(2.*um2(i,j,kp)+um2(i,j-1,kp)+um2(i+1,j-1,kp))
     + +KT(3,np)*(2.*um2(i,j,kp)+um2(i,j-1,kp)+um2(i-1,j  ,kp))
     + +KT(4,np)*(2.*um2(i,j,kp)+um2(i-1,j,kp)+um2(i-1,j+1,kp))
     + +KT(5,np)*(2.*um2(i,j,kp)+um2(i-1,j+1,kp)+um2(i,j+1,kp))
     + +KT(6,np)*(2.*um2(i,j,kp)+um2(i,j+1,kp)+um2(i+1,j  ,kp)))

      FZU=-0.5*S0*( (Sum2+Sum2p)*W(i,j,Kp) -(Sum1+Sum1m)*W(i,j,K) )


      Sum1= c24*
     * (KT(1,n)*(2.*vm2(i,j,k)+vm2(i+1,j,k)+vm2(i+1,j-1,k))
     + +KT(2,n)*(2.*vm2(i,j,k)+vm2(i,j-1,k)+vm2(i+1,j-1,k))
     + +KT(3,n)*(2.*vm2(i,j,k)+vm2(i,j-1,k)+vm2(i-1,j  ,k))
     + +KT(4,n)*(2.*vm2(i,j,k)+vm2(i-1,j,k)+vm2(i-1,j+1,k))
     + +KT(5,n)*(2.*vm2(i,j,k)+vm2(i-1,j+1,k)+vm2(i,j+1,k))
     + +KT(6,n)*(2.*vm2(i,j,k)+vm2(i,j+1,k)+vm2(i+1,j  ,k)))

      Sum1m= c24*
     * (KT(1,n)*(2.*vm2(i,j,km)+vm2(i+1,j,km)+vm2(i+1,j-1,km))
     + +KT(2,n)*(2.*vm2(i,j,km)+vm2(i,j-1,km)+vm2(i+1,j-1,km))
     + +KT(3,n)*(2.*vm2(i,j,km)+vm2(i,j-1,km)+vm2(i-1,j  ,km))
     + +KT(4,n)*(2.*vm2(i,j,km)+vm2(i-1,j,km)+vm2(i-1,j+1,km))
     + +KT(5,n)*(2.*vm2(i,j,km)+vm2(i-1,j+1,km)+vm2(i,j+1,km))
     + +KT(6,n)*(2.*vm2(i,j,km)+vm2(i,j+1,km)+vm2(i+1,j  ,km)))

      Sum2= c24*
     * (KT(1,np)*(2.*vm2(i,j,k)+vm2(i+1,j,k)+vm2(i+1,j-1,k))
     + +KT(2,np)*(2.*vm2(i,j,k)+vm2(i,j-1,k)+vm2(i+1,j-1,k))
     + +KT(3,np)*(2.*vm2(i,j,k)+vm2(i,j-1,k)+vm2(i-1,j  ,k))
     + +KT(4,np)*(2.*vm2(i,j,k)+vm2(i-1,j,k)+vm2(i-1,j+1,k))
     + +KT(5,np)*(2.*vm2(i,j,k)+vm2(i-1,j+1,k)+vm2(i,j+1,k))
     + +KT(6,np)*(2.*vm2(i,j,k)+vm2(i,j+1,k)+vm2(i+1,j  ,k)))

      Sum2p= c24*
     * (KT(1,np)*(2.*vm2(i,j,kp)+vm2(i+1,j,kp)+vm2(i+1,j-1,kp))
     + +KT(2,np)*(2.*vm2(i,j,kp)+vm2(i,j-1,kp)+vm2(i+1,j-1,kp))
     + +KT(3,np)*(2.*vm2(i,j,kp)+vm2(i,j-1,kp)+vm2(i-1,j  ,kp))
     + +KT(4,np)*(2.*vm2(i,j,kp)+vm2(i-1,j,kp)+vm2(i-1,j+1,kp))
     + +KT(5,np)*(2.*vm2(i,j,kp)+vm2(i-1,j+1,kp)+vm2(i,j+1,kp))
     + +KT(6,np)*(2.*vm2(i,j,kp)+vm2(i,j+1,kp)+vm2(i+1,j  ,kp)))
...
*     Metric in Transport
	...
	Transp_Metric_V= +Um2(i,j,k)*Um2(i,j,k)*COS_A/(R*S0)
      ...
      FU=-FXU-FYU+FZU+.5*(DIFU*HZK1+DIFUP*HZ(K))
      FV=-FXV-FYV+FZV+.5*(DIFV*HZK1+DIFVP*HZ(K))
...
*     Metric in Transport
	...
	Transp_Metric_V= +Um2(i,j,k)*Um1(i,j,k)*COS_A/(R*S0)
      ...
      FU=-FXU-FYU+FZU+.5*DIFU*HZ(Km)
      FV=-FXV-FYV+FZV+.5*DIFV*HZ(Km)
...
*     Version 20.04.2001.
      ...
      IF( K .GT. 1) THEN
C     UPPER HALF
      DIFT= DIFT+KT(1,n)*(T(I+1,J,K)-T(I,J,K))/S0
     *       -S2*KT(2,n)*(T(I,J,K)-T(I,J-1,K))*ASR2
      DIFT= DIFT+KT(3,n)*((T(I-1,J,K)-T(I,J,K))/S0
     *               -S3*(T(I,J,K)-T(I,J-1,K))*ASR2)
      DIFT= DIFT+KT(4,n)*(T(I-1,J,K)-T(I,J,K))/S0
     *       -S5*KT(5,n)*(T(I,J,K)-T(I,J+1,K))*ASR2
      DIFT= DIFT+KT(6,n)*((T(I+1,J,K)-T(I,J,K))/S0
     *               -S6*(T(I,J,K)-T(I,J+1,K))*ASR2)
      END IF
      IF( K .LT. KB) THEN
C     LOWER HALF
      DIFTP= DIFTP+KT(1,np)*(T(I+1,J,K)-T(I,J,K))/S0
     *         -S2*KT(2,np)*(T(I,J,K)-T(I,J-1,K))*ASR2
      DIFTP= DIFTP+KT(3,np)*((T(I-1,J,K)-T(I,J,K))/S0
     *               -S3*(T(I,J,K)-T(I,J-1,K))*ASR2)
      DIFTP= DIFTP+KT(4,np)*(T(I-1,J,K)-T(I,J,K))/S0
     *         -S5*KT(5,np)*(T(I,J,K)-T(I,J+1,K))*ASR2
      DIFTP= DIFTP+KT(6,np)*((T(I+1,J,K)-T(I,J,K))/S0
     *               -S6*(T(I,J,K)-T(I,J+1,K))*ASR2)
      END IF
      RETURN
      END
...
*     Version 29.04.2011.
      ...
      IF( K .GT. 1) THEN
C     UPPER HALF
      DIFU=DIFU-2.*COS_A*hx*(KT(1,n)*(v(i+1,j,k)-v(i,j,k))
     *                      +KT(2,n)*(v(i+1,j-1,k)-v(i,j-1,k))
     *                      +KT(3,n)*(v(i,j,k)-v(i-1,j,k))
     *                      +KT(4,n)*(v(i,j,k)-v(i-1,j,k))
     *                      +KT(5,n)*(v(i,j+1,k)-v(i-1,j+1,k))
     *                      +KT(6,n)*(v(i+1,j,k)-v(i,j,k)))/S0

      DIFV=DIFV+2.*COS_A*hx*(KT(1,n)*(u(i+1,j,k)-u(i,j,k))
     *                      +KT(2,n)*(u(i+1,j-1,k)-u(i,j-1,k))
     *                      +KT(3,n)*(u(i,j,k)-u(i-1,j,k))
     *                      +KT(4,n)*(u(i,j,k)-u(i-1,j,k))
     *                      +KT(5,n)*(u(i,j+1,k)-u(i-1,j+1,k))
     *                      +KT(6,n)*(u(i+1,j,k)-u(i,j,k)))/S0
      END IF
      IF( K .LT. KB) THEN
C     LOWER HALF
      DIFUP=DIFUP-2.*COS_A*hx*(KT(1,np)*(v(i+1,j,k)-v(i,j,k))
     *                      +KT(2,np)*(v(i+1,j-1,k)-v(i,j-1,k))
     *                      +KT(3,np)*(v(i,j,k)-v(i-1,j,k))
     *                      +KT(4,np)*(v(i,j,k)-v(i-1,j,k))
     *                      +KT(5,np)*(v(i,j+1,k)-v(i-1,j+1,k))
     *                      +KT(6,np)*(v(i+1,j,k)-v(i,j,k)))/S0

      DIFVP=DIFVP+2.*COS_A*hx*(KT(1,np)*(u(i+1,j,k)-u(i,j,k))
     *                      +KT(2,np)*(u(i+1,j-1,k)-u(i,j-1,k))
     *                      +KT(3,np)*(u(i,j,k)-u(i-1,j,k))
     *                      +KT(4,np)*(u(i,j,k)-u(i-1,j,k))
     *                      +KT(5,np)*(u(i,j+1,k)-u(i-1,j+1,k))
     *                      +KT(6,np)*(u(i+1,j,k)-u(i,j,k)))/S0
      END IF
...
c     and Coriolis force and explicite level gradients.
c     Crank-Nikolson time scheme.
c     Version 25.10.2012.
c-------------------------------------------------------
...
*     Pressure by snow and ice.
	...
	hs=hs+roi   *hice (mg,i,j) ! ice mass
	end do


      PBclin(i,j,1)= PBclin(i,j,1) +g*hs !!!!!-g*row*PME_DZ(i,j)

*     Integration by Z
      DO k=2,KK2
      PBclin(i,j,k)=PBclin(i,j,k-1)+
     &              0.5*g*hz(k-1)*(ro(i,j,k-1)+ro(i,j,k))
      end do
...
cc	Cdn= 3.00 e-3           ! Overland, 1985 (see Leepparanta)
