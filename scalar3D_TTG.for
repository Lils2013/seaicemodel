...
*
*     Explicit time stepping - two step Taylor-Galerkin (TTG)
...
*----------------------------------------------------------------
      ...
      RsT    =0.
*---------------------------------------------------
*         Right Hand Side with NO upwinding
*---------------------------------------------------     
...
*-------------------------------------------------
	...
	kp= k+1
      ...
      call DIFF_Scalar(Sclr2,hz,i,j,k,N,NP,DIFT,DIFTP,KB,
     &                   il1,jl1,kl,dt,KT,ALT)
...
*     --------------- Bottom ------------------
      N = NP
      call DIFF_Scalar(Sclr2,hz,i,j,kb,N,NP,DIFT,DIFTP,KB,
     &               il1,jl1,kl,dt,KT,ALT)
      call transp(FXTK  ,FYTK  ,i,j,kb  ,n,u,v,Sclr2,KT,il1,jl1,kl)
      call transp(FXTKm ,FYTKm ,i,j,kb-1,n,u,v,Sclr2,KT,il1,jl1,kl)
...
c     dT/dz*dfi/dz 
      ...
      SUBROUTINE DIFF_Scalar(T,hz,i,j,k,N,NP,DIFT,DIFTP,KB,
     &                       il,jl,kl,dt,KT,A)

*     Version 30.07.2015.

*     Galerkin diffusion approximation.
*     Ordinary constant coefficient diffusion.
*     Boundary conditions are taken into account in the Main Program.

      dimension T(0:il,0:jl,kl), KT(6,13), hz(kl)
      common /si/ s0,sm,sp,s1,s2,s3,s4,s5,s6,asr,asr2
	common /consts/ c3,c6,c12,c18,Rhx,Rhy,R2hxhy
      real KT
...
c     Triangle # 1.
c     Horizontal part of the operator
      DIFT= DIFT +A*KT(1,n)*c6*(
     & (2.*(T(I+1,J,K)-T(I,J,K))+T(I+1,J,Km)-T(I,J,Km))/asr/S1
     &                          )*hz(km)  

c     Triangle # 2.
c     Horizontal part of the operator
      DIFT= DIFT -A*KT(2,n)*c6*(
     & S2*(2.*(T(I,J,K)-T(I,J-1,K))+T(I,J,Km)-T(I,J-1,Km))*asr
     &                          )*hz(km)

c     Triangle # 3.
c     Horizontal part of the operator
      DIFT= DIFT +A*KT(3,n)*c6*(
     & (2.*(T(I-1,J,K)-T(I,J,K)) +T(I-1,J,Km)-T(I,J,Km) )/asr/S3
     &-S3*asr*(2.*(T(I,J,K)-T(I,J-1,K)) +T(I,J,Km)-T(I,J-1,Km))
     &                          )*hz(km)

c     Triangle # 4.
c     Horizontal part of the operator
      DIFT= DIFT +A*KT(4,n)*c6*(
     & (2.*(T(I-1,J,K)-T(I,J,K))+T(I-1,J,Km)-T(I,J,Km))/asr/S4
     &                          )*hz(km)

c     Triangle # 5.
c     Horizontal part of the operator
      DIFT= DIFT -A*KT(5,n)*c6*(
     &asr*S5*(2.*(T(I,J,K)-T(I,J+1,K))+T(I,J,Km)-T(I,J+1,Km))
     &                          )*hz(km)

c     Triangle # 6.
c     Horizontal part of the operator
      DIFT= DIFT +A*KT(6,n)*c6*(
     & (2.*(T(I+1,J,K)-T(I,J,K)) +T(I+1,J,Km)-T(I,J,Km))/asr/S6
     *-asr*S6*(2.*(T(I,J,K)-T(I,J+1,K)) +T(I,J,Km)-T(I,J+1,Km))
     &                          )*hz(km)
      END IF
...
c     Triangle # 1.
c     Horizontal part of the operator
      DIFTP= DIFTP+A*c6*KT(1,np)*(
     & (2.*(T(I+1,J,K)-T(I,J,K)) +T(I+1,J,Kp)-T(I,J,Kp))/asr/S1
     &                            )*hz(k)

c     Triangle # 2.
c     Horizontal part of the operator
      DIFTP= DIFTP-A*c6*KT(2,np)*(
     & S2*(2.*(T(I,J,K)-T(I,J-1,K))+T(I,J,Kp)-T(I,J-1,Kp))*ASR
     &                            )*hz(k)

c     Triangle # 3.
c     Horizontal part of the operator
      DIFTP= DIFTP+A*c6*KT(3,np)*(
     & (2.*(T(I-1,J,K)-T(I,J,K))+T(I-1,J,Kp)-T(I,J,Kp))/asr/S3
     &-S3*(2.*(T(I,J,K)-T(I,J-1,K))+T(I,J,Kp)-T(I,J-1,Kp))*ASR
     &                            )*hz(k)

c     Triangle # 4.
c     Horizontal part of the operator
      DIFTP= DIFTP+A*c6*KT(4,np)*(
     & (2.*(T(I-1,J,K)-T(I,J,K)) +T(I-1,J,Kp)-T(I,J,Kp))/asr/S4
     &                          )*hz(k)

c     Triangle # 5.
c     Horizontal part of the operator
      DIFTP= DIFTP-A*c6*KT(5,np)*(
     & S5*(2.*(T(I,J,K)-T(I,J+1,K)) +T(I,J,Kp)-T(I,J+1,Kp))*ASR
     &                          )*hz(k)

c     Triangle # 6.
c     Horizontal part of the operator
      DIFTP= DIFTP+A*c6*KT(6,np)*(
     & (2.*(T(I+1,J,K)-T(I,J,K)) +T(I+1,J,Kp)-T(I,J,Kp))/asr/S6
     *-S6*(2.*(T(I,J,K)-T(I,J+1,K)) +T(I,J,Kp)-T(I,J+1,Kp))*ASR
     &                          )*hz(k)
