      SUBROUTINE advect3D(gamma_fct,
     *           Sclr,Sclr1,Sclr2,SclrObs,Qcascad,Agm)

*******************************************************
*
*     Sclr 3D trasport and Isopycnal diffusion.
...
DDD      parameter (gamma_fct=4.e-1)        ! Numerical scalar diffusivity
...
*     Auxiliary arrays for FCT calculations
	...
	dimension Rplus(il,jl,kl), Rminus(il,jl,kl), alpha_e(12)
      dimension antidf(12,il,jl,kl) !Antidifusive fluxes to node (i,j,k)

	real Agm(2,0:il1,0:jl1)  ! Variable GMG coefficient on the prism
...
*-------------------------------------------------
	...
	kp= k+1
      ...
      call DIFFUW_TG_IP(Sclr2,Ropot,u,v,w,hz,i,j,k,N,NP,DIFT,DIFTP,KB,
...
*     ------------------------------------------------------------------
      ...
      RsT(i,j,k)= dt*FT +dt*Qcascad(i,j,k)
...
*     --------------- Bottom ------------------
      N = NP
      call DIFFUW_TG_IP(Sclr2,Ropot,u,v,w,hz,i,j,kb,N,NP,DIFT,DIFTP,KB,
     &               il1,jl1,kl,dt,KT,ALT)
...
c	Tmax= max(Tmax, Sclr2(i,j,km))
c      Tmax= max(Tmax, Sclr2(i,j,k ))
c	Tmin= min(Tmin, Sclr2(i,j,km))
c      Tmin= min(Tmin, Sclr2(i,j,k ))  	
...
c	Tmax= max(Tmax, Sclr2(i,j,kp))
c      Tmax= max(Tmax, Sclr2(i,j,k ))
c	Tmin= min(Tmin, Sclr2(i,j,kp))
c      Tmin= min(Tmin, Sclr2(i,j,k )) 
...
*---------------     Triangle # 1.  ---------------------------------
      FXTK=FXTK+0.75*
     *( (2.*U(I,J,K)+U(I+1,J,K)+U(I+1,J-1,K))*T(I,J,K)
     * +(U(I,J,K)+2.*U(I+1,J,K)+U(I+1,J-1,K))*T(I+1,J,K)
     * +(U(I,J,K)+U(I+1,J,K)+2.*U(I+1,J-1,K))*T(I+1,J-1,K)  )
     * *KT(1,n)
*---------------     Triangle # 2.  -----------------------------------
      FYTK=FYTK-0.75*S2*
     *( (2.*V(I,J,K)+V(I,J-1,K)+V(I+1,J-1,K))*T(I,J,K)
     * +(V(I,J,K)+2.*V(I,J-1,K)+V(I+1,J-1,K))*T(I,J-1,K)
     * +(V(I,J,K)+V(I,J-1,K)+2.*V(I+1,J-1,K))*T(I+1,J-1,K) )
     * *KT(2,n)
*---------------     Triangle # 3.  -------------------------------
      FXTK=FXTK-0.75*
     *( (2.*U(I,J,K)+U(I-1,J,K)+U(I,J-1,K))*T(I,J,K)
     * +(U(I,J,K)+2.*U(I-1,J,K)+U(I,J-1,K))*T(I-1,J,K)
     * +(U(I,J,K)+U(I-1,J,K)+2.*U(I,J-1,K))*T(I,J-1,K)  )
     * *KT(3,n)
     
      FYTK=FYTK-0.75*S3*
     *( (2.*V(I,J,K)+V(I,J-1,K)+V(I-1,J,K))*T(I,J,K)
     * +(V(I,J,K)+2.*V(I,J-1,K)+V(I-1,J,K))*T(I,J-1,K)
     * +(V(I,J,K)+V(I,J-1,K)+2.*V(I-1,J,K))*T(I-1,J,K)  )
     * *KT(3,n)
*---------------     Triangle # 4.  ---------------------------------
      FXTK=FXTK-0.75*
     *( (2.*U(I,J,K)+U(I-1,J,K)+U(I-1,J+1,K))*T(I,J,K)
...
c	end if
	...
	End
     ...
     &                        il,jl,kl,dt,KT,A)
      ...
      parameter (Smax2B=1.e-6) ! Maximum density slope squared
*     AND: Smax**2 < hz**2/(dt*Al) - for the sake of numerical stability      
      
      parameter (puny=1.e-9)

*     Version 04.01.2016.

*     Galerkin artificial diffusion approximation by Taylor-Galerkin
*     Isopycnal diffusion by Redi, 1982.
*     A is the "physical" or rather "background" diffusivity.
*     Boundary conditions are taken into account in the Main Program.

      dimension u(0:il,0:jl,kl), v(0:il,0:jl,kl), w(0:il,0:jl,kl)
      dimension T(0:il,0:jl,kl), KT(6,13), Ro(0:il,0:jl,kl), hz(kl)
...
C     UPPER HALF
      ...
      Smax2= min(Smax2B,0.25*hz(km)/dt/A) ! numerical stability 
...
*     Density slopes 
     ...
     &       (max(1.e-12,(Ro1k-Ro1km))*Rhx*S1)
	ay= 1.5*hz(km)*(Ro(i+1,j,k )-Ro(i+1,j-1,k )+
     &                  Ro(i+1,j,km)-Ro(i+1,j-1,km))/
     &       (max(1.e-12,(Ro1k-Ro1km))*Rhy)

	Smodul= ax**2+ay**2 +puny
	
*     Tapering by Gerdes, et.al., 1991
      if(Smodul .GT. Smax2) then
      ax= ax*Smax2/Smodul	
      ay= ay*Smax2/Smodul	
      Smodul = Smax2	
      end if	
            
*     If mixed layer - no isopicnal diffusion
c      if(Smodul .GT. Smax2) then
c      ax= 0.
c      ay= 0.	
c      Smodul = 0.	
c      end if	
...
C     LOWER HALF
      ...
      Smax2= min(Smax2B,0.25*hz(k)/dt/A) ! - numerical stability 
...
c      if(i.eq.17 .and. j.eq.24 .and. k.eq.1) then
c      write(*,*) a, ax, ay, Smodul
c      end if
