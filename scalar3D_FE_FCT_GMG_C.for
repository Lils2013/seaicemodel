...
*******************************************************
      ...
      parameter (gamma_fct=4.e-1)        ! Numerical scalar diffusivity
...
cc      FZT=-0.5*((Sum1+Sum2)*W(i,j,2)-2.*Sum1*W(i,j,1))
...
c      call SkewFlux(Sclr2,Ropot,hz,i,j,k,N,NP,DIFT,DIFTP,KB,
c     &              il1,jl1,kl,klp,KT,z,Agm)
c      DIFT = A*DIFT
c      DIFTP= A*DIFTP
...
c      call SkewFlux(Sclr2,Ropot,hz,i,j,kb,N,NP,DIFT,DIFTP,KB,
c     &              il1,jl1,kl,klp,KT,z,Agm)
c      DIFT = A*DIFT
c	FT = FT +DIFT
*     ------------------------------------------------------------------
...
c     Horizontal part of the operator
     ...
     &     a11(1)*(T(I+1,J,K)-T(I  ,J,  K))/asr/S1
     &    +a12(1)*(T(i+1,j,k)-T(i+1,j-1,k))         )*hz(km)  
...
c     dT/dz*dfi/dz 
      DIFT= DIFT -KT(1,n)*R2hxhy*S1*a33(1)*(Q1K-Q1Km)/hz(km)
**      DIFT= DIFT -KT(1,n)*R2hxhy*S1*a33(1)*(T(i,j,k)-T(i,j,km))/hz(km)

c     Triangle # 2.

c     Horizontal part of the operator
      DIFT= DIFT -KT(2,n)*0.5*(
     & a22(2)*S2*(T(I,J,K)-T(I,J-1,K))*asr
     &+a21(2)*   (T(i+1,j-1,k)-T(i,j-1,k)))*hz(km)
...
c     dT/dz*dfi/dz 
      DIFT= DIFT -KT(2,n)*R2hxhy*S2*a33(2)*(Q2K-Q2Km)/hz(km)
**      DIFT= DIFT -KT(2,n)*R2hxhy*S2*a33(2)*(T(i,j,k)-T(i,j,km))/hz(km)

c     Triangle # 3.

c     Horizontal part of the operator
      DIFT= DIFT +KT(3,n)*0.5*(
     &         a11(3)*(T(I-1,J,K)-T(I,J,K))/asr/S3
     &    +a12(3)*(T(i,j-1,k)-T(i  ,j,k))
     &    -a21(3)*(T(i,j  ,k)-T(i-1,j,k))
     &-S3*asr*a22(3)*(T(I,J  ,K)-T(I,J-1,K)))*hz(km)
...
c     dT/dz*dfi/dz 
      DIFT= DIFT -KT(3,n)*R2hxhy*S3*a33(3)*(Q3K-Q3Km)/hz(km)
**      DIFT= DIFT -KT(3,n)*R2hxhy*S3*a33(3)*(T(i,j,k)-T(i,j,km))/hz(km)
c      write(*,*) 'point 4'
...
c     Horizontal part of the operator
     ...
     &     a11(4)*(T(I-1,J,K)-T(I,J,K))/asr/S4
     &    +a12(4)*(T(i-1,j,k)-T(i-1,j+1,k)))*hz(km)
...
c     dT/dz*dfi/dz 
      DIFT= DIFT -KT(4,n)*R2hxhy*S4*a33(4)*(Q4K-Q4Km)/hz(km)
**      DIFT= DIFT -KT(4,n)*R2hxhy*S4*a33(4)*(T(i,j,k)-T(i,j,km))/hz(km)

c     Triangle # 5.

c     Horizontal part of the operator
      DIFT= DIFT -KT(5,n)*0.5*(
     &asr*S5*a22(5)*(T(I,J,K)-T(I,J+1,K))
     &  +a21(5)*(T(i-1,j+1,k)-T(i,j+1,k)))*hz(km)
...
c     dT/dz*dfi/dz 
      DIFT= DIFT -KT(5,n)*R2hxhy*S5*a33(5)*(Q5K-Q5Km)/hz(km)
**      DIFT= DIFT -KT(5,n)*R2hxhy*S5*a33(5)*(T(i,j,k)-T(i,j,km))/hz(km)

c     Triangle # 6.

c     Horizontal part of the operator
      DIFT= DIFT +KT(6,n)*0.5*(
     &         a11(6)*(T(I+1,J,K)-T(I,J,K))/asr/S6
     &    +a12(6)*(T(i,j+1,k)-T(i,j,k))   
     &    -a21(6)*(T(i,j,k)-T(i+1,j,k))
     *-asr*S6*a22(6)*(T(I,J,K)-T(I,J+1,K)))*hz(km)
...
c     dT/dz*dfi/dz 
      DIFT= DIFT -KT(6,n)*R2hxhy*S6*a33(6)*(Q6K-Q6Km)/hz(km)
**      DIFT= DIFT -KT(6,n)*R2hxhy*S6*a33(6)*(T(i,j,k)-T(i,j,km))/hz(km)
...
c     Horizontal part of the operator
      DIFTP= DIFTP+0.5*KT(1,np)*(a11(1)*(T(I+1,J,K)-T(I,J,K))/asr/S1
     &            +a12(1)*(T(i+1,j,k)-T(i+1,j-1,k)))*hz(k)
...
c     dT/dz*dfi/dz 
      DIFTP=DIFTP-KT(1,np)*R2hxhy*S1*a33(1)*(Q1K-Q1Kp)/hz(k)
**      DIFTP=DIFTP-KT(1,np)*R2hxhy*S1*a33(1)*(T(i,j,k)-T(i,j,kp))/hz(k)

c     Triangle # 2.

c     Horizontal part of the operator
      DIFTP= DIFTP-0.5*KT(2,np)*(a22(2)*S2*(T(I,J,K)-T(I,J-1,K))*ASR
     &           +        a21(2)*(T(i+1,j-1,k)-T(i,j-1,k)))*hz(k)
...
c     dT/dz*dfi/dz 
      DIFTP=DIFTP-KT(2,np)*R2hxhy*S2*a33(2)*(Q2K-Q2Kp)/hz(k)
**      DIFTP=DIFTP-KT(2,np)*R2hxhy*S2*a33(2)*(T(i,j,k)-T(i,j,kp))/hz(k)

c     Triangle # 3.

c     Horizontal part of the operator
      DIFTP= DIFTP+0.5*KT(3,np)*(a11(3)*(T(I-1,J,K)-T(I,J,K))/asr/S3
     &            +a12(3)*(T(i,j-1,k)-T(i,j,k))
     &            -a21(3)*(T(i,j,k)-T(i-1,j,k))
     &             -S3*a22(3)*(T(I,J,K)-T(I,J-1,K))*ASR)*hz(k)
...
c     dT/dz*dfi/dz 
      DIFTP=DIFTP-KT(3,np)*R2hxhy*S3*a33(3)*(Q3K-Q3Kp)/hz(k)
**      DIFTP=DIFTP-KT(3,np)*R2hxhy*S3*a33(3)*(T(i,j,k)-T(i,j,kp))/hz(k)

c     Triangle # 4.

c     Horizontal part of the operator
      DIFTP= DIFTP+0.5*KT(4,np)*(a11(4)*(T(I-1,J,K)-T(I,J,K))/asr/S4
     &            +a12(4)*(T(i-1,j,k)-T(i-1,j+1,k)))*hz(k)
...
c     dT/dz*dfi/dz 
      DIFTP=DIFTP-KT(4,np)*R2hxhy*S4*a33(4)*(Q4K-Q4Kp)/hz(k)
**      DIFTP=DIFTP-KT(4,np)*R2hxhy*S4*a33(4)*(T(i,j,k)-T(i,j,kp))/hz(k)


c     Triangle # 5.

c     Horizontal part of the operator
      DIFTP= DIFTP-0.5*KT(5,np)*(a22(5)*S5*(T(I,J,K)-T(I,J+1,K))*ASR
     &           +        a21(5)*(T(i-1,j+1,k)-T(i,j+1,k)))*hz(k)
...
c     dT/dz*dfi/dz 
      DIFTP=DIFTP-KT(5,np)*R2hxhy*S5*a33(5)*(Q5K-Q5Kp)/hz(k)
**      DIFTP=DIFTP-KT(5,np)*R2hxhy*S5*a33(5)*(T(i,j,k)-T(i,j,kp))/hz(k)


c     Triangle # 6.

c     Horizontal part of the operator
      DIFTP= DIFTP+0.5*KT(6,np)*(a11(6)*(T(I+1,J,K)-T(I,J,K))/asr/S6
     &               +a12(6)*(T(i,j+1,k)-T(i,j,k))   
     &               -a21(6)*(T(i,j,k)-T(i+1,j,k))
     *               -S6*a22(6)*(T(I,J,K)-T(I,J+1,K))*ASR)*hz(k)
...
c     dT/dz*dfi/dz 
      DIFTP= DIFTP -KT(6,np)*R2hxhy*S6*a33(6)*(Q6K-Q6Kp)/hz(k)
**      DIFTP=DIFTP-KT(6,np)*R2hxhy*S6*a33(6)*(T(i,j,k)-T(i,j,kp))/hz(k)
