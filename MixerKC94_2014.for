...
c	cL0= 0.028*u_star/abs(2.*om*co(i)*S0)     ! McPhee book
	cL0= 0.7  *u_star/abs(2.*om*co(i)*S0)     ! Ekman layer depth ROMS
cccc      cL0= MIN(cL0,1.e3)                    ! 10m CLIO 3.0 default
...
*     Limitation to avoid unphysical results
	...
	ppp= 1.e-5*g*row*z(k)
      ...
      VB= 1.e-3*g*REAL(Ro1-Ro2)/hz(1)/row
	...
	ppp= 5.e-6*g*row*(z(k)+z(km))
      ...
      VB= 1.e-3*g*REAL(Ro1-Ro2)/hz(km)/row
	...
	ppp= 1.e-5*g*row*z(k)
      t12m= 0.5*(t(i,j,km)+t(i,j,k))
	s12m= 0.5*(s(i,j,km)+s(i,j,k))
      t12p= 0.5*(t(i,j,k)+t(i,j,kp))
	s12p= 0.5*(s(i,j,k)+s(i,j,kp))
      ro1=cg(np)*sigma_t(t12p,s12p, ppp)
      ro2=cg(n)*sigma_t(t12m,s12m, ppp)
	VB= 2.e-3*g*REAL(ro1-ro2)/(cg(np)*hz(k)+cg(n)*hz(km))/row
...
*     Bouyoancy fluxes
	...
	ppp= 5.e-6*g*row*(z(k)+z(kp))
      ro1=sigma_t(t(i,j,kp),s(i,j,kp), ppp)
      ro2=sigma_t(t(i,j,k ),s(i,j,k ), ppp)
      VB= 1.e-3*g*REAL(Ro1-Ro2)/hzk/row
     ...
     *	 KT(6,n)*(v(i,j,kp)+v(i+1,j,kp)+v(i,j+1,kp))
      ...
      uz= az(i,j,k)* ( (umkp-umk)/(3.*cg(n)*hzk) )**2
      vz= az(i,j,k)* ( (vmkp-vmk)/(3.*cg(n)*hzk ))**2
	...
	s12m= 0.5*(s(i,j,km)+s(i,j,k))
      t12p= 0.5*(t(i,j,k)+t(i,j,kp))
	s12p= 0.5*(s(i,j,k)+s(i,j,kp))
      ro1=DBLE (cg(np))*sigma_t(t12p,s12p, ppp)
      ro2=DBLE (cg(n ))*sigma_t(t12m,s12m, ppp)
	VB= 2.e-3*g*REAL(ro1-ro2)/(cg(np)*hzk+cg(n)*hzk1)/row
...
*     Vertical shift **2 in velocity points
     ...
     *	 KT(6,np)*(v(i,j,k)+v(i+1,j,k)+v(i,j+1,k))
      ...
      uz= az(i,j,km)* ( (umkpp-umkpk)/(3.*cg(np)*hzk) )**2
     *  + az(i,j,k )* ( (umk-umkm)   /(3.*cg(n) *hzk1))**2
      vz= az(i,j,km)* ( (vmkpp-vmkpk)/(3.*cg(np)*hzk ))**2
     *  + az(i,j,k )* ( (vmk-vmkm)   /(3.*cg(n) *hzk1))**2
...
*     Friction velocity open water
	...
	turb1 =0.5*(q2turb(i,j,1)+q2turb(i,j,2)) 
     ...
     +                     + 2.*Aopen*dt*100.*u_star**3  ! Surface waves
...
*     Vertical shift **2 in velocity points
     ...
     *	KT(6,n)*(v(i,j,km)+v(i+1,j,km)+v(i,j+1,km))
      ...
      uz= az(i,j,k)* ( (umk-umkm)/(3.*cg(n)*hzk1) )**2
      vz= az(i,j,k)* ( (vmk-vmkm)/(3.*cg(n)*hzk1) )**2
