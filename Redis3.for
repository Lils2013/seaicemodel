      subroutine Redis
*     Version 01.10.10 based on the CICE3 model.
*     Variable ice salinity.
*     Suitable for Free Upper Surface.
*     Hibler, 1980, Flato and Hibler 1995.
...
!---!------------------------------------------------------------------
	...
	Pice = 0.            ! Ice strength = 0. 25.12.12
...
c      -----------------------------------------------------------------
        ...
        do n = 1, mgrad
          if (athornn(n) .gt. aimin) then
            hi = Hice(n,i,j)/aice(n,i,j)
            hrminn(n)= 2.*hi             ! Hibler (1980)
