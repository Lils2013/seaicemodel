...
*     Vaisala-Brunt Frequency for 20-30m depth scale
*     Vertical scale
      K_vert_scale = min(kb,7)  ! 5-10m, 7-20m, 8-27.5m, 9-37.5m, 10-50m

	hzk =z(K_vert_scale)

      ro1=ropot(i,j,1)               ! density at 0m depth
      ro2=ropot(i,j,K_vert_scale)    ! density at vert scale depth
      VB30= g*(ro2-ro1)/hzk/row
      VB30= SQRT(MAX(0.,VB30))
      
      IF( VB30 .LE. 1.e-9) then ! Thick upper mixed layer - no wave drag
      
      tauw =0.
      
      else
      
*     Vaisala-Brunt Frequency for 100-m depth scale
*     Vertical scale
cc      K_vert_scale = min(kb,13)  ! k=13 - 100m
      K_vert_scale1 = min(kb,7)  ! k=7  - 20m
      K_vert_scale2 = min(kb,10) ! k=10 - 50m
	hzk1 =z(K_vert_scale1)
	hzk2 =z(K_vert_scale2)
	hzk= hzk2-hzk1
	if (hzk .GT. 0.) then           ! deep water
      ro1=ropot(i,j,K_vert_scale1)    ! density at 20m depth
      ro2=ropot(i,j,K_vert_scale2)    ! density at vert scale depth
      VB100= g*(ro2-ro1)/hzk/row
      VB100= SQRT(MAX(0.,VB100))
      else
      VB100= VB30
      end if
      
      param_nyu= 3.14*delta_u/(max(1.e-9,VB100) *d_ice)
      
      if(param_nyu .LE. 0.2)then  ! K0/Kc   
      tauw= 0.375* 3.14*C100 * hi_variance * VB100
      else
      tauw= 0.075* row *hi_variance * VB100**2 /max(1.e-9, delta_u)
      end if
...
cc      if(i.eq.17.and.j.eq.24) then
cc	write(*,*) i,j,Himean,hi_variance,param_nyu,
cc     *VB30, VB100, delta_u, tauw, row*Cdw*delta_u
cc	end if
...
c     -----------------------------------------------------------------
	...
	CC=(row*Cdw*delta_u+CDgwd(i,j))*dte/MAX(di,eff_thick)
      a11= 1.0+CC
      a22= a11
	a12= -0.5*cor*dte 
	a21= -a12
