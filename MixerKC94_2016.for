...
*     version 20.02.16.
      ...
      INCLUDE 'Slo2.fi'
      dimension AM(kl),BM(kl),CM(kl),FM(kl),Rksi(kl) ! service arrays
	dimension cL(kl)
...
*     As described in CLIO 3.0 manual.
...
*     --------------------------------------------------------------------------
      ...
      Parameter (Sq=0.2)  ! 0.2- Mellor&Yamada, 1982, or Sq=0.41*Sfu
...
c     *(A1=0.58,A2=0.62,B1=16.55,B2=11.6,C1=0.038,C2=0.7,C3=0.2)
	...
	Parameter (cL0=3.e2)    ! macroscale for turbulence, set = 10m 
*                             !	CLIO 3.0 default;
...
c      cL0= MAX(cL0,1.e2)                        ! 10m CLIO 3.0 default
*       cL0= MAX(cL0,1.e3,sigmahice)    
...
cc      cLs= max(cLmin, z(k))              ! distance to the surface
...
c	VB= 2.e-3*g*REAL(ro1-ro2)/(cg(np)*hzk+cg(n)*hzk1)/row
	...
	VB= (cg(n)*azt(i,j,km)*hzk1*VB12 +cg(np)*azt(i,j,k)*hzk*VB32)
     *   /(cg(n)*hzk1+cg(np)*hzk)
...
*     Mean Skin Drag coefficient under ice
	...
	turb1 =max(1.e-3,0.5*(q2turb(i,j,1)+q2turb(i,j,2)))

      AM(1)=0.
      CM(1)=-0.5*CA*CG(n)*(cL(1)+cL(2))*cK*Az(i,j,1)*sqrt(turb1)/hzk
      BM(1)=1.0 -CM(1)     + Pminus
      FM(1)= q2turb(i,j,1) + Pplus  
     +      + 4.*dt*B123*tau2/hz(1)       ! Allen at.al., 1995
	...
	turb1 =max(1.e-3,0.5*(q2turb(i,j,k)+q2turb(i,j,k+1)))
	turb2 =max(1.e-3,0.5*(q2turb(i,j,k-1)+q2turb(i,j,k)))

      AM(k)=-0.5*CA*CG(n )*(cL(k-1)+cL(k  ))*
     &      ck*Az(i,j,k-1)*sqrt(turb2)/hzk1
      CM(k)=-0.5*CA*CG(np)*(cL(k  )+cL(k+1))*
     &       cK*Az(i,j,k+1)*sqrt(turb1) /hzk
...
*     Vertical shift **2 in velocity points consistent with momentum equation
	...
	turb2 =max(1.e-3,0.5*(q2turb(i,j,kb-1)+q2turb(i,j,kb)))
	
      AM(kb)=-0.5*CA*(cL(kb)+cL(kb-1))*
     &        cK*Az(i,j,kb-1)*sqrt(turb2)/hz(kb-1)
      CM(kb)= 0.
      BM(kb)= 1.-AM(kb)      +Pminus
      FM(kb)= q2turb(i,j,kb) +Pplus 
     * + 4.*dt*B123*(tauxb**2 +tauyb**2)/hz(kb-1) ! Allen, et.al.,1995
...
c      if( VB.LT.0.) then ! T,S Kantha and Clayson (1994) - 50
c      Az (i,j,k)= 1.e3
c      azt(i,j,k)= 1.e3
c      azs(i,j,k)= 1.e3
c      end if

c	if(z(k) .LE. 20.e2) then
c      Az (i,j,k)= Az (i,j,k) +100.
c      AzT(i,j,k)= AzT(i,j,k) +100.
c      AzS(i,j,k)= AzS(i,j,k) +100.
c	end if
...
c	if(0.5*(z(k)+z(kp)) .LE. 2.0*himean) then
c      Az (i,j,k)= 1.e3
c      AzT(i,j,k)= 1.e3
c      AzS(i,j,k)= 1.e3
c	end if
...
*     Double diffusion - requires special treatment of bouyancy frequency!
	...
	Tk12= 0.5*(T(i,j,k)+T(i,j,kp))
	Sk12= 0.5*(S(i,j,k)+S(i,j,kp))
	Pk12= 0.5e-5*g*row*(z(k)+z(kp))


      RoS=  sigma_t(Tk12,S(i,j,kp),Pk12)-sigma_t(Tk12,S(i,j,k),Pk12)
      RoT=  sigma_t(T(i,j,kp),Sk12,Pk12)-sigma_t(T(i,j,k),Sk12,Pk12)

	if(RoT .GE. 0.) then
	R_Ro= RoS/max( puny,RoT)
	else
	R_Ro= RoS/min(-puny,RoT)
	end if

	IF(R_Ro .LE. 1.9 .AND. R_Ro .GE. 1.0) then

c	write(*,*) 'Double diffusion happened! i,j,k,R_ro,add', 
c     *                    i,j,k,R_Ro,add

	add = 50. * (1. - ((R_Ro-1.)/0.9)**2)**3
      AzT(i,j,k)= AzT(i,j,k) + 0.7*add
      AzS(i,j,k)= AzS(i,j,k) +     add
...
c     Neutral bouyancy level
     ...
     &    Ropot(i,j,m+1).GT.Ropot(i,j,k)) exit
      end do 
...
cc      vconv = u_stokes *1.e-2   ! 1 plume D=100m per 1x1 sq.km
      vconv = u_stokes            ! 1 giant plume
      vconv = MIN(0.25*hz(k)/dt,vconv)       ! Stability criterium
      Uplume= cg(np)*R2hxhy*vconv        ! Volume flux by plumes
