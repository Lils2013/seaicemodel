...
*     Cranck-Nichelsen Time Scheme.
*     Introduced Ekman rotation 25 degrees.
*     Variable air drug coefficient
...
c     -----Parameters for ice-ocean drag  ---------
      Fcr= 1.0        !! Critical Froude number - limits the stress?
	CDf= 3.0        !! Blocked flow form drag: 1-7
	Hnc= 0.4        !! Parameter for blocked flow 0.4 for 2D
	d_ice= 300.e2   !! mean icefloe diameter, 300m
      C100= row/d_ice
...
c     ----- Surface pressure and initial data for drift -----
	...
	himean= 0.
	do mg=1,mgrad
	hs    =hs    +rosdry*hsnow(mg,i,j)
	hs    =hs    +roi   *hice (mg,i,j)
...
*     Surface pressure ice growth have no influence (???)

****	serv(i,j)=10.*PA(i,j)+g*(row*(REAL(dz(i,j))-PME_DZ(i,j)) +hs)


	serv(i,j)=10.*PA(i,j) + g*(row*REAL(dz(i,j)) +hs)
...
*     Vertical scale
      K_vert_scale = 5               ! 5 - 10m, 7 -20m, 10 - 50m

	hzk =z(K_vert_scale)

      ro1=ropot(i,j,1)               ! density at 0m depth
      ro2=ropot(i,j,K_vert_scale)    ! density at vert scale depth
...
c     -----------------Wake effect-----------------------------
      ALF= d_ice*SQRT(Aice(0,i,j)/max(1.e-9,1.-Aice(0,i,j)))
	ALF= 1./max(1.e-9,ALF)
c     ---------------------------------------------------------


c     --------- Sum over sea ice thickness gradations ---------
	do m=1,mgrad
...
c      Hscale= 100.
c      Cdw1=  Cdw1 + 
c     *aice(m,i,j)*( 0.4/alog(Hscale*300./(max(5.,0.5*hi))) )**2

*     Gravity wave drag 
	dumax= Fcr*ABS(hi-himean)*VB
	dumax=MIN(delta_u,dumax)/delta_u
	tauw= tauw +Aice(m,i,j) *dumax *(hi-himean)**2
	
*     Blocked flow and skin drag
      Hn= (VB+1.e-9)*hi/delta_u
      Zb= hi*MAX(0.,1.0 -Hnc/Hn)
      taub= taub +Aice(m,i,j)*Zb

*     Wake effect
      Gamma= Gamma + Aice(m,i,j)*(1.-SQRT(hi*ALF))**2
...
*     ---------------------------------------------------------
      ...
      Gamma= Gamma/max(1.e-9, 1.-Aice(0,i,j)) ! Wake effect


c     ---------------------------------------------------------
	CDgwd(i,j) =   row*Cdw*delta_u
c     *             + c100*VB*tauw
c     *             + 2.*c100*CDf*taub*delta_u  !* Gamma
c     ---------------------------------------------------------


c      if(i.eq.17.and.j.eq.24) then
c	write(*,*)c100*VB*tauw,2.*c100*CDf*taub*delta_u,row*Cdw*delta_u
c	end if
...
*     Some auxillary functions are everywhere.
      if(abs_n.GT.0) call EVP(i,j,k,nstep,fu_evp,fv_evp,dte,Tdamp)


ccc      if(ice_mask(i,j).EQ.1) then 
ccc      if(ice_mask(i,j).EQ.1 .and. n.LE.1 .and. n.NE.0) then 
      if(ice_mask(i,j).EQ.1 .and. n.EQ.1) then ! Radiation
...
c      Q=1.+1.6e-4*(row/roa)*SQRT(Aice(0,i,j)*(1.-Aice(0,i,j)))
c      windxi= Q*windxi
c      windyi= Q*windyi
c     -----------------------------------------------------------------

	CC=CDgwd(i,j)*dte/MAX(di,eff_thick)
      a11= 1.0+CC
      a22= a11
	a12= -0.5*cor*dte 
	a21= -a12
...
*     Water Friction.
      Fu= uice2(i,j)-a12*vice2(i,j)+CC*u(i,j,1)
	Fv= vice2(i,j)+a12*uice2(i,j)+CC*v(i,j,1)
...
cc	C_Damp= 61.5*Tdamp*R*R*hx*hy*S0/dte/dte 
