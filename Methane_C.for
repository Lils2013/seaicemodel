...
*     Suitable for Flather, 1976, radiation condition.
      ...
      parameter (hxgr=1., hygr=1., dthr=0.5, finord=23., alwest=-16.)
      parameter (ncond=1)
...
*     For Real*8 - 2 times more
	...
	dimension MEC(12)
      ...
      real Agm(2,0:il1,0:jl1)   ! Variable GMG coeff in the prizm
...
*     Tides, Sea level
	...
	DIMENSION DAMPL4(5),DPH4(5)  ! Incoming Tidal wave - Bering Pas.
      ...
      real medalb, medsf, medice, medpnd, asnow, Apn, alpn
...
*     Mainly for numerical purposes
*     Lower limit is molecular diffusion/viscosity
*    ----------------------------------
      do k=1,kl
      azbg(k)  = +1.5e-2  +1.e-8* hz(k)**2   
      aztbg(k) = +1.3e-3  +1.e-9* hz(k)**2    
      azsbg(k) = +1.3e-5  +1.e-9* hz(k)**2   
      end do
*    ----------------------------------
...
c     Methane and oxygen data
     ...
     *      form='unformatted',recl=nnn)
      open (unit=201,file="medalb.txt",action="write",
     * status="old")
...
*     Canadian Passages
	...
	dzext(10,28)= -40.              ! Nares
	dzext(3,16:17) = dzext_mean(3,16)    -10. ! McClure
	dzext(6,17:22) = dzext_mean(6,17:22) -10. ! Sverdrup Basin. 
*	See the ref.:
*     Melling, H., Sea ice of the northern Canadian Arctic Archipelago, 
*     J. Geophys. Res., 107(C11), 3181, doi:10.1029/2001JC001102, 2002.
...
*     Velocity
	...
	do k=1,km2(10,28)
	ub(10,28,k)= ub(10,28,k) -4.00*T_new/(Tide_M2*dt)
	end do
*     Sea level
	T_new= cos(               105.89*pi/180.)-
     *       cos(-Tide_M2*dt +105.89*pi/180.)
	dzext(10,28)= dzext(10,28) -37.39*T_new/(Tide_M2*dt)
...
*     Alaska-Canadian
	...
	do k=1,km2(3,16)
	ub(3,16:17,k)= ub(3,16:17,k) -3.06*T_new/(Tide_M2*dt)
	end do
*     Sea level
	T_new= cos(               275.39*pi/180.)-
     *       cos(-Tide_M2*dt +275.39*pi/180.)
	dzext(3,16)= dzext(3,16) -10.17*T_new/(Tide_M2*dt)
...
c	hi= 0.
c      do mg=1,mgrad
c	hi=hi+rosdry*hsnow(mg,i,j)/row ! snow mass
c	hi=hi+roi   *hice (mg,i,j)/row ! ice mass
c	end do
c      dz(i,j) = -DBLE(hi)
c	dzm(i,j)= dz(i,j)
c	END IF            ! kk2>0
...
c     Level at open boundary
      do i=11,23
	dzext(i,jl)= 2.5*cos((imonth-10.)*pi/6.) 
     *+ 1.0*dzext_mean(i,jl)  + 5. !+1.0e-3*(i-11.)**4! Norwegian Sea
	end do

      do j=43,46
	dzext(7,j) = 2.5*cos((imonth-10.)*pi/6.) 
     *+ 1.0*dzext_mean(7,j)   -35. !! -1.5*(j-43.)**2! Denmark Strait
	end do

	dzext(:,1) = 2.5*cos((imonth-8. )*pi/6.)
     *+ 1.0*dzext_mean(:,1)   !+10.    ! Bering Passage
...
*     Ice-ocean relative velocity

      do j=1,jl
      do i=1,il
      delta_u(i,j)=
     * SQRT((uice(i,j)-u(i,j,1))**2+(vice(i,j)-v(i,j,1))**2)
      end do
      end do
...
c     Density in situ
      ...
      do K=1,km2(i,j)        
      ppp= 1.e-5* g*row*z(k) !!+1.e-4*PA(i,j)                
      ro(i,j,k)= 1.e-3*sigma_t(t(i,j,k),s(i,j,k), ppp)
      end do
      end if
      end do
      end do
...
c     Potential density.
      ...
      if( km2(i,j) .GT. 0) then
      do K=1,km2(i,j)
      ropot(i,j,k)= 1.e-3*sigma_t(t(i,j,k),s(i,j,k), 0.)
      end do
      end if
      end do
      end do

      call UB_SLO(ub,unept,vnept,nt3,km2,dzext,Pext,PA,wx,wy,
     *            tobs,sobs,az,il,jl,il1,jl1,kl,klp,
     *            aice,u,v,uice,vice,CDgwd,mgrad,
     *            hx,hy,r,si,co,hz,z,g,om,row,roa,CDw)

      call GMG_Visbeck_Coeffs (Agm) ! Variable GMG coeff on the prism

c     ---------------------------------------------------------

c     Turbulence Closer Scheme   

      Sclr2= Q2turb
	Sclr1= 0.
	SclrObs= 0. 
	Qcascad= 0.
***	call advect3D(0.4,Sclr,Sclr1,Sclr2,SclrObs,Qcascad,Agm)
...
c	if(kb.GT.0) then
c	do k=1,kb
c	Q2turb(i,j,k)= max( Sclr(i,j,k), 0.)
c	end do
c	end if
c	end do
c	end do
...
* ======== Temperature and Salinity transport =======
      ...
      QSc= 0.
c      call Cascading
cc      call TS_convection
cc      call TS_convection_ice
...
c      if(kb.gt.0) then
c      do k=1,kb
c      QTctotal= QTctotal +QTc(i,j,k)
c      QSctotal= QSctotal +QSc(i,j,k)
c      end do
c      end if
c      end do
c      end do
      
c      write(*,*) 'Convection = ', QTctotal, QSctotal
	...
	SclrObs=Tobs
	Qcascad = QTc 
	!call advect3D(0.4,Sclr,Sclr1,Sclr2,SclrObs,Qcascad,Agm) !ìîæíî
	!T= Sclr                                                 !âûêëþ÷èòü
...
c    +++++++++++++++++++ NEW DENSITY IN SITU and PRESSURE ++++++

c      do j=1,jl
c      do i=1,il
c      if( km2(i,j) .GT. 0) then
c      do K=1,km2(i,j)
c	ppp= 1.e-5* g*row*z(k)
c      ro(i,j,k)= 1.e-3*sigma_t(t(i,j,k),s(i,j,k), ppp)
c      end do
c      end if
c      end do
c      end do
...
****      call tr(iYear,iMonth,u,v,km2,si,hz,il1,jl1,kl,0)
      ...
      medalb=0
      do mgg=1,mgrad
      if (aice(mgg,12,11).GT.1.0e-6) then
      asnow=F_as(Tsnow(mgg,12,11))
	...
	write(*,*)'BG S model and data, 5m =',s(12,13,1),sobs(12,13,1)
	write(*,*)'NP S model and data, 5m =',s(17,24,1),sobs(17,24,1)
	write(*,*)'S(12,24) model and data, 5m =',
     *          s(12,24,1),sobs(12,24,1)
	write(*,*)'T,S Norwegian',
     *          T(20,49,1),Tobs(20,49,1),S(20,49,1),Sobs(20,49,1)
      call maxu
...
*     Mean Skin-Drag coefficient
      i=17
      j=24
	tau1= (uice(i,j)-um2(i,j,1))**2
	tau2= (vice(i,j)-vm2(i,j,1))**2
	
	A50cm =0. ! Ice compactness Hice>50cm
	do m=4,mgrad
	A50cm= A50cm +Aice(m,i,j)
	end do
...
***      CTb=1.5 e-3              ! Mellor&Kantha, 1989
      TFC= TFr(S(i,j,1), 0.) ! Freezing T at the depth of ice bottom
...
*     version 12.12.2014 for big arrays methane + oxygen case
...
88    format('<<<< Data are stored!!!', i4,i3,' >>>>')
      ...
      subroutine Neptune(L)

*     Version 12.02.16 - Kazantsev. et.al., 1998, Polyakov, 2001

	include 'Slo2.fi'
      common /diffus/ al,alt
      
      RN= 5.e5       ! R Neptune = 5km, G. Holloway.
**      RN= 3.e5       ! Rn= 3km

	alpha= 3.0 ! To account for resolution - stream width 30km 
*     Al_eff= 4U(H/nablaH)*1/f*1/hx**2*Al, 
*     U~Rn**2*f*nabla(H)/H ~ 2.5-5cm/s, Rn ~ 3-5km, so
*     Al_eff = 4* (Rn/hx)**2 *Al

	do j=1,jl
      S0=Si(j)
      
*     Polyakov - Al=2.e8, hx= 55.6km - balance of Neptune and viscosity      
      Al_eff=2.*AL*(RN**2)*( 1./hx**2 +1./(hy*S0)**2)/(R**2)
**      Al_eff= 2.*AL*(RN/(R*sqrt(hx*hy*S0)))**2
**      Al_eff= AL*(RN/(R*sqrt(hx*hy*S0)))**2
**      Al_eff= AL
      
**      Al_eff= 1.e-4*RN**2  ! f*R**2
	...
	uneptF= -alpha*(L*dt)*Al_eff*cor*unept(i,j)
	vneptF= -alpha*(L*dt)*Al_eff*cor*vnept(i,j)
	
***	det = 1. /(1. + dt*Al_eff/RN**2)
	
***	write(*,*) 'det=', det, uneptF, vneptF
     
	do k=1,km2(i,j) 

*     Kazantsev's style
c     		u(i,j,k)= det*(u(i,j,k) +uneptF)
c     		v(i,j,k)= det*(v(i,j,k) +vneptF)

*     Polyakov's style		
		u(i,j,k)= u(i,j,k) +uneptF
		v(i,j,k)= v(i,j,k) +vneptF
	...
	return
      ...
      subroutine scalar_lumped

*     Lumped matrix for temperature and salinity transport scheme
	...
	sumkp=
     *c6*(KT(1,np)*S1+KT(2,np)*S2+KT(3,np)*S3
	...
	sumk=
     *c6*(KT(1,n)*S1+KT(2,n)*S2+KT(3,n)*S3
	...
	sclamp(i,j,k)= 0.5*(HZK1*sumk +HZK*sumkp)
