      SUBROUTINE iceadvect(gamma_fct,Scalar,Scalar1,Scalar2,uice,vice,
     *RSice,ice_mask,nt3,si,KT,il,jl,il1,
     *jl1,kl,minit,mgrad,r,hx,hy,dt)
...
ccccc      parameter (gamma_fct=1.0e-2)        ! Numerical diffusivity
      parameter (Adiff= 5.0e6)           ! Background diffusivity
...
*---------------------------------------------------     
     ...
     *                il1,jl1,mgrad)
     ...
     &                                         mgrad,KT,Adiff,dt)
      ...
      RsICE(MG,i,j)=dt*Fa
      if (nst .eq. 6) then
      !write(*,*) FXa, "FXa", Fa, "Fa", FYa, "FYa", DIFFa, "DIFFa"
      end if
	Scalar1(mg,i,j)= 0. !!!RsIce(mg,i,j) /Alump
...
*----------------------------------------------------------------
     ...
     *            + omega*(RsICE(mg,i,j)-sum) / Alump
      if (nst .eq. 6) then
      !write(*,*) Scalar(mg,i,j), "Scalar(mg,i,j)", mg, i, j
      !write(*,*) (RsICE(mg,i,j)), "omega", Alump
      end if
...
****	if(nt3(i,j,1).NE.0 .and. ice_mask(i,j).eq.1) then !!all points
	do mg=minit,mgrad
...
*---------------     Triangle # 1.  ---------------------------------
      FXTK=FXTK+0.75*
     *( (2.*U(I,J)+U(I+1,J)+U(I+1,J-1))*T(mg,i  ,J  )
     * +(U(I,J)+2.*U(I+1,J)+U(I+1,J-1))*T(mg,i+1,J  )
     * +(U(I,J)+U(I+1,J)+2.*U(I+1,J-1))*T(mg,i+1,J-1)  )
     * *KT(1,n)
*---------------     Triangle # 2.  -----------------------------------
      FYTK=FYTK-0.75*S2*
     *( (2.*V(I,J)+V(I,J-1)+V(I+1,J-1))*T(mg,i  ,J  )
     * +(V(I,J)+2.*V(I,J-1)+V(I+1,J-1))*T(mg,i  ,J-1)
     * +(V(I,J)+V(I,J-1)+2.*V(I+1,J-1))*T(mg,i+1,J-1)  )
     * *KT(2,n)
*---------------     Triangle # 3.  -------------------------------
      FXTK=FXTK-0.75*
     *( (2.*U(I,J)+U(I-1,J)+U(I,J-1))*T(mg,i  ,J  )
     + +(U(I,J)+2.*U(I-1,J)+U(I,J-1))*T(mg,i-1,J  )
     * +(U(I,J)+U(I-1,J)+2.*U(I,J-1))*T(mg,i  ,J-1)   )
     * *KT(3,n)
     
      FYTK=FYTK-0.75*S3*
     *( (2.*V(I,J)+V(I,J-1)+V(I-1,J))*T(mg,i  ,J  )
     * +(V(I,J)+2.*V(I,J-1)+V(I-1,J))*T(mg,i  ,J-1)
     * +(V(I,J)+V(I,J-1)+2.*V(I-1,J))*T(mg,i-1,J  )   )
     * *KT(3,n)
*---------------     Triangle # 4.  ---------------------------------
      FXTK=FXTK-0.75*
     *( (2.*U(I,J)+U(I-1,J)+U(I-1,J+1))*T(mg,i  ,J  )
