...
*     -------------- Surface -----------------
	...
	sumkp=
     *c4*(KT(1,np)*(2.*Tm2(i,j,k)+Tm2(i+1,j,k)+Tm2(i+1,j-1,k))
     +   +KT(2,np)*(2.*Tm2(i,j,k)+Tm2(i,j-1,k)+Tm2(i+1,j-1,k))
     +   +KT(3,np)*(2.*Tm2(i,j,k)+Tm2(i,j-1,k)+Tm2(i-1,j  ,k))
     +   +KT(4,np)*(2.*Tm2(i,j,k)+Tm2(i-1,j,k)+Tm2(i-1,j+1,k))
     +   +KT(5,np)*(2.*Tm2(i,j,k)+Tm2(i-1,j+1,k)+Tm2(i,j+1,k))
     +   +KT(6,np)*(2.*Tm2(i,j,k)+Tm2(i,j+1,k)+Tm2(i+1,j  ,k)))
	sumkpp=
     *c4*(KT(1,np)*(2.*Tm2(i,j,kp)+Tm2(i+1,j,kp)+Tm2(i+1,j-1,kp))
     +   +KT(2,np)*(2.*Tm2(i,j,kp)+Tm2(i,j-1,kp)+Tm2(i+1,j-1,kp))
     +   +KT(3,np)*(2.*Tm2(i,j,kp)+Tm2(i,j-1,kp)+Tm2(i-1,j  ,kp))
     +   +KT(4,np)*(2.*Tm2(i,j,kp)+Tm2(i-1,j,kp)+Tm2(i-1,j+1,kp))
     +   +KT(5,np)*(2.*Tm2(i,j,kp)+Tm2(i-1,j+1,kp)+Tm2(i,j+1,kp))
     +   +KT(6,np)*(2.*Tm2(i,j,kp)+Tm2(i,j+1,kp)+Tm2(i+1,j  ,kp)))
...
*     --------------  Deep water -------------
	...
	sumk=
     *c4*(KT(1,n)*(2.*Tm2(i,j,k)+Tm2(i+1,j,k)+Tm2(i+1,j-1,k))
     +   +KT(2,n)*(2.*Tm2(i,j,k)+Tm2(i,j-1,k)+Tm2(i+1,j-1,k))
     +   +KT(3,n)*(2.*Tm2(i,j,k)+Tm2(i,j-1,k)+Tm2(i-1,j  ,k))
     +   +KT(4,n)*(2.*Tm2(i,j,k)+Tm2(i-1,j,k)+Tm2(i-1,j+1,k))
     +   +KT(5,n)*(2.*Tm2(i,j,k)+Tm2(i-1,j+1,k)+Tm2(i,j+1,k))
     +   +KT(6,n)*(2.*Tm2(i,j,k)+Tm2(i,j+1,k)+Tm2(i+1,j  ,k)))

	sumkm=
     *c4*(KT(1,n)*(2.*Tm2(i,j,km)+Tm2(i+1,j,km)+Tm2(i+1,j-1,km))
     +   +KT(2,n)*(2.*Tm2(i,j,km)+Tm2(i,j-1,km)+Tm2(i+1,j-1,km))
     +   +KT(3,n)*(2.*Tm2(i,j,km)+Tm2(i,j-1,km)+Tm2(i-1,j  ,km))
     +   +KT(4,n)*(2.*Tm2(i,j,km)+Tm2(i-1,j,km)+Tm2(i-1,j+1,km))
     +   +KT(5,n)*(2.*Tm2(i,j,km)+Tm2(i-1,j+1,km)+Tm2(i,j+1,km))
     +   +KT(6,n)*(2.*Tm2(i,j,km)+Tm2(i,j+1,km)+Tm2(i+1,j  ,km)))


      FZT=-r2s12*((sumkpp+sumkp)*W(I,J,K+1)
     -           -(  sumk+sumkm)*W(I,J,K) 
     * )
      END IF
      ...
      RsT(i,j,k)= CA*FT
cc	Tm1(i,j,k)= omega*RsT(i,j,k)/sclamp(i,j,k)
...
*     --------------- Bottom ------------------
	...
	sumk=
     *c4*(KT(1,n)*(2.*Tm2(i,j,kb)+Tm2(i+1,j,kb)+Tm2(i+1,j-1,kb))
     +   +KT(2,n)*(2.*Tm2(i,j,kb)+Tm2(i,j-1,kb)+Tm2(i+1,j-1,kb))
     +   +KT(3,n)*(2.*Tm2(i,j,kb)+Tm2(i,j-1,kb)+Tm2(i-1,j  ,kb))
     +   +KT(4,n)*(2.*Tm2(i,j,kb)+Tm2(i-1,j,kb)+Tm2(i-1,j+1,kb))
     +   +KT(5,n)*(2.*Tm2(i,j,kb)+Tm2(i-1,j+1,kb)+Tm2(i,j+1,kb))
     +   +KT(6,n)*(2.*Tm2(i,j,kb)+Tm2(i,j+1,kb)+Tm2(i+1,j  ,kb)))
     ...
     *c4*(KT(1,n)*(2.*Tm2(i,j,km)+Tm2(i+1,j,km)+Tm2(i+1,j-1,km))
     ...
     +   +KT(6,n)*(2.*Tm2(i,j,km)+Tm2(i,j+1,km)+Tm2(i+1,j  ,km)))
      ...
      RsT(I,J,KB)= CA*FT
cc	Tm1(i,j,kb)= omega*RsT(i,j,kb)/sclamp(i,j,kb)
...
c	if(i.eq.19.and.j.eq.48.and.k.eq.10) 
c     *write(*,*)iter,T(i,j,k),Tm2(i,j,k) !!!,sum,RsT(i,j,k),sclamp(i,j,k)
...
1000  continue
	...
	do j=1,jl
	do i=1,il
	if(nt3(i,j,k).GT.0) T(i,j,k)=Tm2(i,j,k)+Tm1(i,j,k)
	end do
	end do
	end do
...
*     Version 16.10.2003.
      ...
      FYTK= 0.
*---------------     Triangle # 1.  ---------------------------------
c      FXTK=FXTK+(U(I,J,K)+U(I+1,J,K)+U(I+1,J-1,K))*
c     *          (T(I  ,J  ,K)+T(I+1,J  ,K)+T(I+1,J-1,K))
c     * *KT(1,n)
*---------------     Triangle # 2.  -----------------------------------
c      FYTK=FYTK-S2*(V(I,J,K)+V(I,J-1,K)+V(I+1,J-1,K))*
c     *             (T(I  ,J  ,K)+T(I  ,J-1,K)+T(I+1,J-1,K))
c     *  *KT(2,n)
*---------------     Triangle # 3.  -------------------------------
c      FXTK=FXTK-(U(I,J,K)+U(I-1,J,K)+U(I,J-1,K))*
c     *          (T(I  ,J  ,K)+T(I-1,J  ,K)+T(I  ,J-1,K))
c     *  *KT(3,n)
c      FYTK=FYTK-S3*(V(I,J,K)+V(I,J-1,K)+V(I-1,J,K))*
c     *             (T(I  ,J  ,K)+T(I  ,J-1,K)+T(I-1,J  ,K))
c     *  *KT(3,n)
*---------------     Triangle # 4.  ---------------------------------
c      FXTK=FXTK-(U(I,J,K)+U(I-1,J,K)+U(I-1,J+1,K))*
c     *          (T(I  ,J  ,K)+T(I-1,J  ,K)+T(I-1,J+1,K))
c     *  *KT(4,n)
*---------------     Triangle # 5.  ----------------------------------
c      FYTK=FYTK+S5*(V(I,J,K)+V(I,J+1,K)+V(I-1,J+1,K))*
c     *             (T(I  ,J  ,K)+T(I  ,J+1,K)+T(I-1,J+1,K))
c     *  *KT(5,n)
*---------------     Triangle # 6.  -----------------------------------
c      FXTK=FXTK+(U(I,J,K)+U(I+1,J,K)+U(I,J+1,K))*
c     *          (T(I  ,J  ,K)+T(I+1,J  ,K)+T(I  ,J+1,K))
c     *  *KT(6,n)
c      FYTK=FYTK+S6*(V(I,J,K)+V(I,J+1,K)+V(I+1,J,K))*
c     *             (T(I  ,J  ,K)+T(I  ,J+1,K)+T(I+1,J  ,K))
c     *  *KT(6,n)
...
*---------------     Triangle # 1.  ---------------------------------
      FXTK=FXTK+((2.*U(I,J,K)+U(I+1,J,K)+U(I+1,J-1,K))*T(I  ,J  ,K) +
     +           (2.*U(I+1,J,K)+U(I,J,K)+U(I+1,J-1,K))*T(I+1,J  ,K) +
     +           (2.*U(I+1,J-1,K)+U(I+1,J,K)+U(I,J,K))*T(I+1,J-1,K))
     * *KT(1,n)
*---------------     Triangle # 2.  -----------------------------------
      FYTK=FYTK-S2*((2.*V(I,J,K)+V(I,J-1,K)+V(I+1,J-1,K))*T(I  ,J  ,K)+
     +              (2.*V(I,J-1,K)+V(I,J,K)+V(I+1,J-1,K))*T(I  ,J-1,K)+
     +              (2.*V(I+1,J-1,K)+V(I,J,K)+V(I,J-1,K))*T(I+1,J-1,K))
     *  *KT(2,n)
*---------------     Triangle # 3.  -------------------------------
      FXTK=FXTK-((2.*U(I,J,K)+U(I-1,J,K)+U(I,J-1,K))*T(I  ,J  ,K) +
     +           (2.*U(I-1,J,K)+U(I,J,K)+U(I,J-1,K))*T(I-1,J  ,K) +
     +           (2.*U(I,J-1,K)+U(I-1,J,K)+U(I,J,K))*T(I  ,J-1,K))
     *  *KT(3,n)
      FYTK=FYTK-S3*((2.*V(I,J,K)+V(I,J-1,K)+V(I-1,J,K))*T(I  ,J  ,K) +
     +              (2.*V(I,J-1,K)+V(I,J,K)+V(I-1,J,K))*T(I  ,J-1,K) +
     +              (2.*V(I-1,J,K)+V(I,J,K)+V(I,J-1,K))*T(I-1,J  ,K))
     *  *KT(3,n)
*---------------     Triangle # 4.  ---------------------------------
      FXTK=FXTK-((2.*U(I,J,K)+U(I-1,J,K)+U(I-1,J+1,K))*T(I  ,J  ,K) +
     +           (2.*U(I-1,J,K)+U(I,J,K)+U(I-1,J+1,K))*T(I-1,J  ,K) +
     ...
     *  *KT(4,n)
*---------------     Triangle # 5.  ----------------------------------
      FYTK=FYTK+S5*((2.*V(I,J,K)+V(I,J+1,K)+V(I-1,J+1,K))*T(I  ,J  ,K)+
     +              (2.*V(I,J+1,K)+V(I,J,K)+V(I-1,J+1,K))*T(I  ,J+1,K)+
     +              (2.*V(I-1,J+1,K)+V(I,J,K)+V(I,J+1,K))*T(I-1,J+1,K))
     *  *KT(5,n)
*---------------     Triangle # 6.  -----------------------------------
     ...
     +            (2.*U(I,J+1,K)+U(I+1,J,K)+U(I,J,K))*T(I  ,J+1,K))
     *  *KT(6,n)
      FYTK=FYTK+S6*((2.*V(I,J,K)+V(I,J+1,K)+V(I+1,J,K))*T(I  ,J  ,K) +
     +              (2.*V(I,J+1,K)+V(I,J,K)+V(I+1,J,K))*T(I  ,J+1,K) +
     +              (2.*V(I+1,J,K)+V(I,J,K)+V(I,J+1,K))*T(I+1,J  ,K))
     *  *KT(6,n)
