...
*     Auxiliary arrays for FCT calculations
	...
	dimension Rplus(il,jl,kl), Rminus(il,jl,kl), alpha_e(12)
      dimension antidf(12,il,jl,kl) !Antidif fluxes to node (i,j,k) on elements 
...
c	if(i.eq.19 .and. j.eq.5.and.k.eq.1) write(*,*)'init ',Tm1(i,j,k)
...
cc	if(i.eq.9.and.j.eq.41.and.k.eq.kb) write(*,*) 'init:', Tm1(i,j,kb)
...
*----------------------------------------------------------------
	...
	sumk=c24*(KT(1,n)*S1*(2.*Tm2(i,j,k)+Tm2(i+1,j,k)+Tm2(i+1,j-1,k))
     +         +KT(2,n)*S2*(2.*Tm2(i,j,k)+Tm2(i,j-1,k)+Tm2(i+1,j-1,k))
     +         +KT(3,n)*S3*(2.*Tm2(i,j,k)+Tm2(i,j-1,k)+Tm2(i-1,j  ,k))
     +         +KT(4,n)*S4*(2.*Tm2(i,j,k)+Tm2(i-1,j,k)+Tm2(i-1,j+1,k))
     +         +KT(5,n)*S5*(2.*Tm2(i,j,k)+Tm2(i-1,j+1,k)+Tm2(i,j+1,k))
     +         +KT(6,n)*S6*(2.*Tm2(i,j,k)+Tm2(i,j+1,k)+Tm2(i+1,j  ,k)))
	...
	sumkp=
     *c24*(KT(1,np)*S1*(2.*Tm2(i,j,k)+Tm2(i+1,j,k)+Tm2(i+1,j-1,k))
     +    +KT(2,np)*S2*(2.*Tm2(i,j,k)+Tm2(i,j-1,k)+Tm2(i+1,j-1,k))
     +    +KT(3,np)*S3*(2.*Tm2(i,j,k)+Tm2(i,j-1,k)+Tm2(i-1,j  ,k))
     +    +KT(4,np)*S4*(2.*Tm2(i,j,k)+Tm2(i-1,j,k)+Tm2(i-1,j+1,k))
     +    +KT(5,np)*S5*(2.*Tm2(i,j,k)+Tm2(i-1,j+1,k)+Tm2(i,j+1,k))
     +    +KT(6,np)*S6*(2.*Tm2(i,j,k)+Tm2(i,j+1,k)+Tm2(i+1,j  ,k)))
	sumkpp=
     *c24*(KT(1,np)*S1*(2.*Tm2(i,j,kp)+Tm2(i+1,j,kp)+Tm2(i+1,j-1,kp))
     +    +KT(2,np)*S2*(2.*Tm2(i,j,kp)+Tm2(i,j-1,kp)+Tm2(i+1,j-1,kp))
     +    +KT(3,np)*S3*(2.*Tm2(i,j,kp)+Tm2(i,j-1,kp)+Tm2(i-1,j  ,kp))
     +    +KT(4,np)*S4*(2.*Tm2(i,j,kp)+Tm2(i-1,j,kp)+Tm2(i-1,j+1,kp))
     +    +KT(5,np)*S5*(2.*Tm2(i,j,kp)+Tm2(i-1,j+1,kp)+Tm2(i,j+1,kp))
     +    +KT(6,np)*S6*(2.*Tm2(i,j,kp)+Tm2(i,j+1,kp)+Tm2(i+1,j  ,kp)))
	...
	hzk1=hz(km)
	sumkm=
     *c24*(KT(1,n)*S1*(2.*Tm2(i,j,km)+Tm2(i+1,j,km)+Tm2(i+1,j-1,km))
     +    +KT(2,n)*S2*(2.*Tm2(i,j,km)+Tm2(i,j-1,km)+Tm2(i+1,j-1,km))
     +    +KT(3,n)*S3*(2.*Tm2(i,j,km)+Tm2(i,j-1,km)+Tm2(i-1,j  ,km))
     +    +KT(4,n)*S4*(2.*Tm2(i,j,km)+Tm2(i-1,j,km)+Tm2(i-1,j+1,km))
     +    +KT(5,n)*S5*(2.*Tm2(i,j,km)+Tm2(i-1,j+1,km)+Tm2(i,j+1,km))
     +    +KT(6,n)*S6*(2.*Tm2(i,j,km)+Tm2(i,j+1,km)+Tm2(i+1,j  ,km)))
...
c      antidf(1,i,j,k)= (gamma_fct*Sum1+Sum2) 
c     *         / (r2s12*(CG(n)*HZK1+CG(np)*HZK)) ! Lamped mass matrix inverted
...
c      antidf(2,i,j,k)= (gamma_fct*Sum1+Sum2) 
c     *         / (r2s12*(CG(n)*HZK1+CG(np)*HZK)) ! Lamped mass matrix inverted
...
c      antidf(3,i,j,k)= (gamma_fct*Sum1+Sum2) 
c     *         / (r2s12*(CG(n)*HZK1+CG(np)*HZK)) ! Lamped mass matrix inverted
...
c      antidf(4,i,j,k)= (gamma_fct*Sum1+Sum2) 
c     *         / (r2s12*(CG(n)*HZK1+CG(np)*HZK)) ! Lamped mass matrix inverted
...
c      antidf(5,i,j,k)= (gamma_fct*Sum1+Sum2) 
c     *         / (r2s12*(CG(n)*HZK1+CG(np)*HZK)) ! Lamped mass matrix inverted
...
c      antidf(6,i,j,k)= (gamma_fct*Sum1+Sum2) 
c     *         / (r2s12*(CG(n)*HZK1+CG(np)*HZK)) ! Lamped mass matrix inverted
...
*     Admitted max/min on the stencil
	...
	Tmin=  1.e19
      ...
      if(KT(1,n) .GT. 0.) then	
	Tmax= max(Tmax, T(i,j,km),T(i+1,j,km),T(i+1,j-1,km))
	Tmax= max(Tmax, T(i,j,k ),T(i+1,j,k ),T(i+1,j-1,k ))
	Tmin= min(Tmin, T(i,j,km),T(i+1,j,km),T(i+1,j-1,km))
	Tmin= min(Tmin, T(i,j,k ),T(i+1,j,k ),T(i+1,j-1,k ))
*     Additional limitation like Zalesak
	Tmax= max(Tmax, Tm2(i,j,km),Tm2(i+1,j,km),Tm2(i+1,j-1,km))
	Tmax= max(Tmax, Tm2(i,j,k ),Tm2(i+1,j,k ),Tm2(i+1,j-1,k ))
	Tmin= min(Tmin, Tm2(i,j,km),Tm2(i+1,j,km),Tm2(i+1,j-1,km))
	Tmin= min(Tmin, Tm2(i,j,k ),Tm2(i+1,j,k ),Tm2(i+1,j-1,k ))
	end if

      if(KT(2,n) .GT. 0.) then	
	Tmax= max(Tmax, T(i,j,km),T(i,j-1,km),T(i+1,j-1,km))
	Tmax= max(Tmax, T(i,j,k ),T(i,j-1,k ),T(i+1,j-1,k ))
	Tmin= min(Tmin, T(i,j,km),T(i,j-1,km),T(i+1,j-1,km))
	Tmin= min(Tmin, T(i,j,k ),T(i,j-1,k ),T(i+1,j-1,k ))
*     Additional limitation like Zalesak
	Tmax= max(Tmax, Tm2(i,j,km),Tm2(i,j-1,km),Tm2(i+1,j-1,km))
	Tmax= max(Tmax, Tm2(i,j,k ),Tm2(i,j-1,k ),Tm2(i+1,j-1,k ))
	Tmin= min(Tmin, Tm2(i,j,km),Tm2(i,j-1,km),Tm2(i+1,j-1,km))
	Tmin= min(Tmin, Tm2(i,j,k ),Tm2(i,j-1,k ),Tm2(i+1,j-1,k ))
	end if

      if(KT(3,n) .GT. 0.) then	
	Tmax= max(Tmax, T(i,j,km),T(i-1,j,km),T(i,j-1,km))
	Tmax= max(Tmax, T(i,j,k ),T(i-1,j,k ),T(i,j-1,k ))
	Tmin= min(Tmin, T(i,j,km),T(i-1,j,km),T(i,j-1,km))
	Tmin= min(Tmin, T(i,j,k ),T(i-1,j,k ),T(i,j-1,k ))
*     Additional limitation like Zalesak
	Tmax= max(Tmax, Tm2(i,j,km),Tm2(i-1,j,km),Tm2(i,j-1,km))
	Tmax= max(Tmax, Tm2(i,j,k ),Tm2(i-1,j,k ),Tm2(i,j-1,k ))
	Tmin= min(Tmin, Tm2(i,j,km),Tm2(i-1,j,km),Tm2(i,j-1,km))
	Tmin= min(Tmin, Tm2(i,j,k ),Tm2(i-1,j,k ),Tm2(i,j-1,k ))
	end if
...
*     Additional limitation like Zalesak
	...
	Tmax= max(Tmax, T(i,j,kp),T(i+1,j,kp),T(i+1,j-1,kp))
	Tmax= max(Tmax, T(i,j,k ),T(i+1,j,k ),T(i+1,j-1,k ))
	Tmin= min(Tmin, T(i,j,kp),T(i+1,j,kp),T(i+1,j-1,kp))
	Tmin= min(Tmin, T(i,j,k ),T(i+1,j,k ),T(i+1,j-1,k ))
*     Additional limitation like Zalesak
	Tmax= max(Tmax, Tm2(i,j,kp),Tm2(i+1,j,kp),Tm2(i+1,j-1,kp))
	Tmax= max(Tmax, Tm2(i,j,k ),Tm2(i+1,j,k ),Tm2(i+1,j-1,k ))
	Tmin= min(Tmin, Tm2(i,j,kp),Tm2(i+1,j,kp),Tm2(i+1,j-1,kp))
	Tmin= min(Tmin, Tm2(i,j,k ),Tm2(i+1,j,k ),Tm2(i+1,j-1,k ))
	end if

      if(KT(2,np) .GT. 0.) then	
	Tmax= max(Tmax, T(i,j,kp),T(i,j-1,kp),T(i+1,j-1,kp))
	Tmax= max(Tmax, T(i,j,k ),T(i,j-1,k ),T(i+1,j-1,k ))
	Tmin= min(Tmin, T(i,j,kp),T(i,j-1,kp),T(i+1,j-1,kp))
	Tmin= min(Tmin, T(i,j,k ),T(i,j-1,k ),T(i+1,j-1,k ))
*     Additional limitation like Zalesak
	Tmax= max(Tmax, Tm2(i,j,kp),Tm2(i,j-1,kp),Tm2(i+1,j-1,kp))
	Tmax= max(Tmax, Tm2(i,j,k ),Tm2(i,j-1,k ),Tm2(i+1,j-1,k ))
	Tmin= min(Tmin, Tm2(i,j,kp),Tm2(i,j-1,kp),Tm2(i+1,j-1,kp))
	Tmin= min(Tmin, Tm2(i,j,k ),Tm2(i,j-1,k ),Tm2(i+1,j-1,k ))
	end if

      if(KT(3,np) .GT. 0.) then	
	Tmax= max(Tmax, T(i,j,kp),T(i-1,j,kp),T(i,j-1,kp))
	Tmax= max(Tmax, T(i,j,k ),T(i-1,j,k ),T(i,j-1,k ))
	Tmin= min(Tmin, T(i,j,kp),T(i-1,j,kp),T(i,j-1,kp))
	Tmin= min(Tmin, T(i,j,k ),T(i-1,j,k ),T(i,j-1,k ))
*     Additional limitation like Zalesak
	Tmax= max(Tmax, Tm2(i,j,kp),Tm2(i-1,j,kp),Tm2(i,j-1,kp))
	Tmax= max(Tmax, Tm2(i,j,k ),Tm2(i-1,j,k ),Tm2(i,j-1,k ))
	Tmin= min(Tmin, Tm2(i,j,kp),Tm2(i-1,j,kp),Tm2(i,j-1,kp))
	Tmin= min(Tmin, Tm2(i,j,k ),Tm2(i-1,j,k ),Tm2(i,j-1,k ))
	end if
...
*     Element #1
	...
	a3=1.0
      ...
      a1= min(a1,Rplus (i,j,k))
	else
      a1= min(a1,Rminus(i,j,k))
      end if
      if(antidf(3,i+1,j,k) .GT. 0.) then
      a2= min(a2,Rplus (i+1,j,k))
	else
      a2= min(a2,Rminus(i+1,j,k))
      end if
      if(antidf(5,i+1,j-1,k) .GT. 0.) then
      a3= min(a3,Rplus (i+1,j-1,k))
	else
      a3= min(a3,Rminus(i+1,j-1,k))
      end if
...
*     Element #2
	...
	a3=1.0
      ...
      a2= min(a2,Rplus (i+1,j-1,k))
	else
      a2= min(a2,Rminus(i+1,j-1,k))
      end if
      if(antidf(6,i,j-1,k) .GT. 0.) then
      a3= min(a3,Rplus (i,j-1,k))
	else
      a3= min(a3,Rminus(i,j-1,k))
      end if
...
*     Element #3
	...
	a3=1.0
      ...
      a2= min(a2,Rplus (i,j-1,k))
	else
      a2= min(a2,Rminus(i,j-1,k))
      end if
      if(antidf(1,i-1,j,k) .GT. 0.) then
      a3= min(a3,Rplus (i-1,j,k))
	else
      a3= min(a3,Rminus(i-1,j,k))
      end if
...
*----------------------------------------------------------------
      do nelem=1,12
        ...
        T(i,j,k)= T(i,j,k) + alpha_e(nelem)*antidf(nelem,i,j,k)
