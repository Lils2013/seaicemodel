...
c     Version 23.07.2015 for ice thickness gradations.
...
**    Temperature flux due to Rain and Rivers and Ridging
...
*     Mean Skin-Drag coefficient Cdw
	tau1= (uice(i,j)-um2(i,j,1))**2
	tau2= (vice(i,j)-vm2(i,j,1))**2

*     Friction velocity
      u_star=SQRT(Cdw*(tau1+tau2))
...
c	Qsi= c1*(c2*TFC-(1.-c1)*Tmelt_s)/(c1+c2)   !another form -no arrays
...
***      if(i.eq.17.and.j.eq.24) write(*,*) m,Hice(m,i,j),dHice,Qiw,Qi
...
c  2. New ice formation on open water.
...
**	F_as=0.80 -0.125*(T+1.0)  ! AOMIP modified
